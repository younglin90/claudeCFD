"""Minimal: just EB4 baseline vs Shapiro."""
import sys, os
...
import time
for diss, coef in [('none', 0.0), ('shapiro', 0.1), ('shapiro', 0.3)]:
    t0 = time.time()
    p_n, u_n = run_eb4(diss, coef)
    nyq, d2 = metrics(p_n, 1e5)
    print(f"{diss} {coef}: Nyquist={nyq:.2e}, d2_rms={d2:.2e}, time={time.time()-t0:.1f}s")

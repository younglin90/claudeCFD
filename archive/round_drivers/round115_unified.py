"""Round 115: Unified config with acoustic_method='auto' for both 02-A and 07-B.
...
# Round 115 unified config — single set, EOS-aware auto-switch internally
UNIFIED_BASE = dict(
    cfl=0.9,
    time_integrator='strang',  # R115: outer_richardson 은 strang 분기에만 활성
    acoustic_method='auto',     # NASG→imex_5n, SG→im1
    primitive_recon='auto',     # NASG→none, SG→tvd (Round 115 신규)
    alpha_scheme='thinc_bvd',
    acid_interface=True,
    dissipation='none',
    strang_richardson=True,     # Round 97: 5-7% wave preservation
    im1_theta=0.5,    # Round 110: CN base for SG (BE damping 제거)
    outer_richardson='auto',  # Round 115: SG auto-enable, NASG disable
)
...
def run_02A():
    ...
    plt.suptitle(f'Round 115 — 02-A NASG (auto→imex_5n, dt=0.01) — ep={ep:.2e}, {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
...
def run_07B(name, ph1c, ph2c, x_intf, sigma_L, x_src, t_end):
    ...
    for i, (lab, num, ex) in enumerate([('p', pf, p_ex), ('u', uf, u_ex), ('rho_mix', rho_num, rho_ex), ('a1', a1f, a1_ex)]):
        ...
        axes[1,i].set_title(f'|num-exact| {lab}'); axes[1,i].grid()
    plt.suptitle(f'Round 115 — 07 {name} (auto→im1, acoustic CFL=0.4) — Lip={Lip:.3f} {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
...
if __name__ == '__main__':
    ...
    with open('results/round115_results.txt','w') as f: f.write('\n'.join(results)+'\n')

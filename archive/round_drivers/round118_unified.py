"""Round 118: Two-speed Suliciu advective face state (Birke-Chalons-Klingenberg 2023).
Based on round114_unified.py — only advective_flux='suliciu' added.
- 02-A NASG → solver auto-picks imex_5n (unchanged from R114)
- 07-B SG/Ideal → solver auto-picks im1, Suliciu face velocity replaces SLAU2
- Suliciu: u^* = (a_L*u_L + a_R*u_R + p_L - p_R) / (a_L + a_R), a_K = Z_K + floor
"""
...
# Round 118 unified config — Suliciu advective face state added
UNIFIED_BASE = dict(
    cfl=0.9,
    ...
    im1_theta=0.5,              # Round 114: CN base for SG (BE damping 제거)
    advective_flux='suliciu',   # R118: Birke 2021 Z-aware face state
)
...
def run_02A():
    ...
    plt.suptitle(f'Round 118 — 02-A NASG (Suliciu, auto→imex_5n, dt=0.01) — ep={ep:.2e}, {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
    plt.savefig('results/1D/02_A/round118_diff_vs_exact.png', dpi=120)
    plt.close()
    print(f'Plot saved: results/1D/02_A/round118_diff_vs_exact.png', flush=True)
    return msg, PASS
...
def run_07B(name, ph1c, ph2c, x_intf, sigma_L, x_src, t_end):
    ...
    for i, (lab, num, ex) in enumerate([('p', pf, p_ex), ('u', uf, u_ex), ('rho_mix', rho_num, rho_ex), ('a1', a1f, a1_ex)]):
        ...
        axes[1,i].set_title(f'|num-exact| {lab}'); axes[1,i].grid()
    plt.suptitle(f'Round 118 — 07 {name} (Suliciu, auto→im1, acoustic CFL=0.9) — Lip={Lip:.3f} {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
    plt.savefig(f'{case_dir}/round118_diff_vs_exact.png', dpi=120)
    plt.close()
    print(f'Plot saved: {case_dir}/round118_diff_vs_exact.png', flush=True)
    return msg, PASS
...
if __name__ == '__main__':
    ...
    with open('results/round118_results.txt','w') as f: f.write('\n'.join(results)+'\n')
    print('\nAll results written to results/round118_results.txt', flush=True)

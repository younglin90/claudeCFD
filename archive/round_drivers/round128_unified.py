"""Round 128: Defect-Correction IM1 (Wesseling 1992 §5.4).

Adds _peluchon_acoustic_im1_dc: predictor (BE) + 1-pass trapezoidal
corrector using mid-state Q_mid = 0.5*(Q_n + Q_pred).

Changes vs R127:
- im1_dc=True  explicitly set (also auto-on for im1 fallback path via auto dispatch)
- im1_dc_corrector_steps=1 (1-pass corrector, default)
- 02-A NASG: imex_5n path — DC NOT entered, bit-identical regression guard
- argon-air: lagrange_projection path — DC NOT entered, bit-identical
- helium-air, air-water: im1 fallback path — DC ACTIVE

Theory: BE amplitude factor ≈ 1 − σ²ρc²k²dx² (p-row dominant damping).
DC 1-pass: factor ≈ 1 − σ⁴ρ²c⁴k⁴dx⁴/4 (2nd-order time).
Ref: Wesseling 1992 Multigrid §5.4; Hairer-Wanner 1996 ODE II §IV.8.
"""
...
# Round 128 unified config — DC IM1 active for im1 fallback path
UNIFIED_BASE = dict(
    cfl=0.9,
    time_integrator='auto',
    acoustic_method='auto',         # NASG→imex_5n, c_ratio≤1.5→lag_hllc, else→im1+DC
    primitive_recon='teno5a',       # NASG→none (auto), SG/Ideal→tvd
    ...
    im1_dc=True,                    # R128: DC corrector ON (auto also sets this for im1 path)
    im1_dc_corrector_steps=1,       # 1-pass corrector
)
...
def run_02A():
    ...
    plt.suptitle(f'Round 128 — 02-A NASG (auto→imex_5n, DC NOT entered) — ep={ep:.2e}, {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
    plt.savefig('results/1D/02_A/round128_diff_vs_exact.png', dpi=120)
    plt.close()
    print(f'Plot saved: results/1D/02_A/round128_diff_vs_exact.png', flush=True)
    return msg, PASS
...
def run_phase1():
    ...
    plt.suptitle(f'Round 128 — Phase 1 (im1+DC, SG water-air) — ep={ep:.2e} {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
    plt.savefig('results/1D/phase1/round128_phase1.png', dpi=120)
    plt.close()
    print(f'Plot saved: results/1D/phase1/round128_phase1.png', flush=True)
    return msg, PASS
...
def run_07B(name, ph1c, ph2c, x_intf, sigma_L, x_src, t_end):
    ...
    for i, (lab, num, ex) in enumerate([('p', pf, p_ex), ('u', uf, u_ex), ('rho_mix', rho_num, rho_ex), ('a1', a1f, a1_ex)]):
        ...
        axes[1,i].set_title(f'|num-exact| {lab}'); axes[1,i].grid()
    plt.suptitle(f'Round 128 — 07 {name} (im1+DC, CFL=0.9) — Lip={Lip:.3f} Liu={Liu:.3f} {"PASS" if PASS else "FAIL"}')
    plt.tight_layout()
    plt.savefig(f'{case_dir}/round128_diff_vs_exact.png', dpi=120)
    plt.close()
    print(f'Plot saved: {case_dir}/round128_diff_vs_exact.png', flush=True)
    return msg, PASS
...
if __name__ == '__main__':
    ...
    # Phase 1 regression (DC enters; uniform p,u → corrector defect=0 → bit-identical)
    msg_p1, p_p1 = run_phase1(); results.append(msg_p1); print(msg_p1, flush=True)

    # 02-A: NASG regression guard (DC NOT entered — imex_5n branch)
    ...
    with open('results/round128_results.txt','w') as f: f.write('\n'.join(results)+'\n')
    print('\nAll results written to results/round128_results.txt', flush=True)

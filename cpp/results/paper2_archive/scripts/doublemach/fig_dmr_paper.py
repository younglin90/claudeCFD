# -*- coding: utf-8 -*-
"""Double Mach reflection on the paper mesh, t = 0.2, 370 421 triangles.
...
G = np.load(os.path.join(S, "dmp_grid.npz"))
gx, gy = G["gx"], G["gy"]
...
LINES = np.linspace(1.7, 21.0, 30)

def style(ax):
    ax.set_aspect("equal")
    ax.set_xlim(gx[0], gx[-1]); ax.set_ylim(gy[0], gy[-1])
    ax.set_xlim(0.0, 3.0)                       # x > 3 is undisturbed pre-shock state
    ax.set_xticks([0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]); ax.set_yticks([0, 0.5, 1.0])
    ax.set_xticks(np.arange(0, 3.01, 0.1), minor=True)
...
for k, tag in (("s1", "tanh"), ("s2", "closed")):
    ...
    style(ax)
    fig.savefig(os.path.join(OUT, f"Fig_dmp_density_{tag}.png"))
    fig.savefig(os.path.join(OUT, f"Fig_dmp_density_{tag}.pdf"))
    plt.close(fig)
    print(f"saved Fig_dmp_density_{tag}")

D = np.abs(G["s1_rho"] - G["s2_rho"])
fig, ax = plt.subplots(figsize=(13.0, 5.0))
im = ax.pcolormesh(gx, gy, np.maximum(D, 1e-5), cmap="inferno_r",
                   norm=LogNorm(vmin=1e-2, vmax=3.0), shading="auto", rasterized=True)
ax.contour(gx, gy, G["s1_rho"], levels=[4.0, 10.0, 17.0], colors="#2E7DB5",
           linewidths=0.4, alpha=0.85)
style(ax)
cb = fig.colorbar(im, ax=ax, fraction=0.022, pad=0.018)
...
fig.savefig(os.path.join(OUT, "Fig_dmp_diff.png"))
fig.savefig(os.path.join(OUT, "Fig_dmp_diff.pdf"))
plt.close(fig)
print("saved Fig_dmp_diff")

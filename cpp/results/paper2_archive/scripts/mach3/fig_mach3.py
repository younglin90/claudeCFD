# -*- coding: utf-8 -*-
"""Mach-3 forward-facing step, t = 4, 129 024 triangles on [0,3]x[0,1].
...
G = np.load(os.path.join(S, "m3_grid.npz"))
gx, gy = G["gx"], G["gy"]
...
for k, tag in (("s1", "tanh"), ("s2", "closed")):
    ...
    style(ax)
    fig.savefig(os.path.join(OUT, f"Fig_m3_density_{tag}.png"))
    fig.savefig(os.path.join(OUT, f"Fig_m3_density_{tag}.pdf"))
    plt.close(fig)
    print(f"saved Fig_m3_density_{tag}")

D = np.abs(G["s1_rho"] - G["s2_rho"])
fig, ax = plt.subplots(figsize=(13.0, 5.0))
im = ax.pcolormesh(gx, gy, np.maximum(D, 1e-5), cmap="inferno_r",
                   norm=LogNorm(vmin=1e-2, vmax=2.0), shading="auto", rasterized=True)
ax.contour(gx, gy, G["s1_rho"], levels=[1.0, 2.5, 4.5], colors="#2E7DB5",
...
fig.savefig(os.path.join(OUT, "Fig_m3_diff.png"))
fig.savefig(os.path.join(OUT, "Fig_m3_diff.pdf"))
plt.close(fig)
print("saved Fig_m3_diff")

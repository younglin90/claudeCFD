# -*- coding: utf-8 -*-
"""Load the paper2 shock-vortex dumps (S1, S2, S3) and resample onto a uniform grid."""
import numpy as np, os
import matplotlib.tri as mtri

R = os.path.join(RESULTS, "paper2_3_data", "shockvortex")
import os
...
raw = {}
for k in ("s1", "s2", "s3"):
    pts, cells, f = read_vtk(os.path.join(R, k, "field.txt.vtk"))
    print(f"{k}: points={len(pts)} cells={cells.shape} fields={sorted(f)} "
          f"rho=[{f['rho'].min():.4f}, {f['rho'].max():.4f}]")
    raw[k] = f
    if "pts" not in raw:
...
print(f"\ndomain x [{x0:.4f}, {x1:.4f}]  y [{y0:.4f}, {y1:.4f}]  "
...
for k in ("s1", "s2"):
    ...
    h = gy[1] - gy[0]
    du_dy, du_dx = np.gradient(np.nan_to_num(out[f"{k}_u"]), h, gx[1] - gx[0])
    dv_dy, dv_dx = np.gradient(np.nan_to_num(out[f"{k}_v"]), h, gx[1] - gx[0])
    out[f"{k}_vort"] = dv_dx - du_dy
    print(f"{k}: grid {NX}x{NY}  rho [{np.nanmin(out[f'{k}_rho']):.4f}, "
          f"{np.nanmax(out[f'{k}_rho']):.4f}]  |w|max {np.abs(out[f'{k}_vort']).max():.1f}")
...
for lab, m in (("|w| below median", w < np.percentile(w, 50)),
    ...
    print(f"   {lab:18s} mean|d rho| = {np.nanmean(D[m]):.4e}   "
          f"share of squared diff = {100*np.nansum(D[m]**2)/np.nansum(D**2):5.1f}%")

np.savez_compressed(os.path.join(S, "sv_grid.npz"), **out)
print("\ncached -> sv_grid.npz")

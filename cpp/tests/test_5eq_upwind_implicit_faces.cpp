...
int main() {
    std::ifstream input(UPWIND_FACE_REF);
    if (!input) return 1;
    std::vector<double> p_ref,u_ref; std::string line;
    while (std::getline(input,line)) { if (line.empty() || line[0]=='#') continue;
        std::istringstream row(line); int i; double p,u; row>>i>>p>>u; p_ref.push_back(p);u_ref.push_back(u); }
    ...
    const auto face=implicit_face_pu(W,eos1,eos2,BC5::Periodic,BC5::Periodic,
                                     .2,true,false,0.,.1,1.e-8,false,true);
    double worst=0.; for(std::size_t i=0;i<p_ref.size();++i) {
    ...
    std::printf("upwind implicit faces oracle max %.3e\\n",worst);

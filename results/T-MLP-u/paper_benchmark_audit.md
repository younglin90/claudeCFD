...
Generated from the committed high-resolution artifacts in `results/T-MLP-u`.
...
| Shock OFF is pure SUPERBEE with `mlp_bound=False` | comparison definition; Double Mach OFF NaN at `t=0.0470621834`; Mach 3 step OFF NaN at `t=0.0123635385` | PASS |
...
| Mach 3 step reaches `t=4` robustly with no OFF-style NaN | ON row `ok=true`, `steps=10740`; OFF row `ok=false` | PASS |
| Double Mach reaches `t=0.2` robustly with no OFF-style NaN | ON row `ok=true`, `steps=3912`; OFF row `ok=false` | PASS |
| T-MLP-u beats all listed unstructured limiters on Double Mach vortex sharpness | ON `vortex_proxy=0.20761`; MLP-u1 `0.25330`, MLP-u2 `0.24939` | FAIL |
| T-MLP-u beats all listed unstructured limiters on Mach 3 flag metric | ON `flag_proxy=0.09340`; MLP-u1 `0.09959`, MLP-u2 `0.09713` | FAIL |
| Overall manifest says complete paper evidence is ready | `paper_benchmark_summary.json`: `evidence_ready=0`, `double_mach_ready=0` | FAIL |
...
| T-MLP-u OFF | NaN | 0.0 | - |
| T-MLP-u ON | ok | 0.20761 | 0.004868 |
...
| T-MLP-u OFF | NaN | 0.0 | - |
| T-MLP-u ON | ok | 0.09340 | 0.017495 |

## Audit Conclusion

The current artifacts demonstrate the intended robustness story: pure
downwind/SUPERBEE OFF diverges, while the T-MLP-u wrapper remains finite on
the paper-scale triangle meshes.  They do not yet prove the stronger paper
claim that T-MLP-u is sharper or better than MLP-u1/MLP-u2 on the shock
benchmarks.  The active research task should remain open until Double Mach
and Mach 3 step sharpness metrics are improved or the paper claim is narrowed
to a robustness/cleanliness tradeoff.

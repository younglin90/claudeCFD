# solver/denner_1d/he2024_solver.py
# He2024 5-equation fully coupled implicit solver.
#
...
#
# Key insight: α₁ρ₁ = ρY₁ (partial density) — same as Fraysse rhoY1.
# Therefore, mixture_eos_anp(α₁ρ₁, α₂ρ₂, ρu, ρE, ph1, ph2) applies directly.
#
# Ref: CLAUDE.md Phase 2, Allaire-Massoni (2002), Kapila et al. (2001)

import numpy as np
from .fraysse_common import (
    anp, _AD, ad_jacobian,
    _get_ph_params, mixture_eos_anp,
    hllc_flux_anp, rusanov_flux_anp, physical_flux_anp,
...
# ---------------------------------------------------------------------------
# MMACM: interface detector κ (Eq. 57, He & Tan 2024)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# MMACM: full correction flux (Eqs. 55-56, He & Tan 2024)
# ---------------------------------------------------------------------------

def _mmacm_correction_flux(a1, a1r1, a2r2, p, T, u_vel, u_face,
                            a1_face_upwind, ph1, ph2, bc_l, bc_r,
                            mmacm_beta=2.9, kappa_q=2):
    """Compute MMACM sharpening correction fluxes G at all faces (N+1,).

    Step 1: Interface indicator kappa (cell-center → face = min(L, R))
    Step 2: Harmonic limiter downwind α₁ face (ᾰ₁)
    Step 3: G^α₁ = κ̃ * u * (ᾰ₁ - α₁_upwind)
    Step 4: Conservation consistency (Eq. 55):
              G^{α₁ρ₁} = ρ̃₁ * G^α₁
              G^{α₂ρ₂} = -ρ̃₂ * G^α₁
              G^{ρu}   = (ρ̃₁ - ρ̃₂) * û * G^α₁
              G^{ρE}   = (ρ̃₁·Ẽ₁ - ρ̃₂·Ẽ₂) * G^α₁

    All quantities tilde (~) are upwind-biased.

    Ref: He & Tan 2024, JCP 513:113192, Eqs. 55-56.
    """
    _eps = 1e-300
    N = len(a1)

    # --- kappa at cell centers → face (min of left and right) ---
    kappa_cell = _kappa_anp(a1, bc_l, bc_r, q_exp=kappa_q)
    kappa_ext  = _ghost_anp(kappa_cell, bc_l, bc_r)  # (N+2,)
    kappa_face = anp.minimum(kappa_ext[0:N+1], kappa_ext[1:N+2])  # (N+1,)

    # --- Downwind α₁ face via harmonic limiter ---
    a1_down = _harmonic_limiter_face_anp(a1, u_face, bc_l, bc_r,
                                          beta_lim=mmacm_beta)  # (N+1,)

    # --- G^α₁ (Eq. 56) ---
    G_alpha = kappa_face * u_face * (a1_down - a1_face_upwind)  # (N+1,)
    ...
    rho1_up = a1r1_up / anp.maximum(a1_up,        1e-10)
    rho2_up = a2r2_up / anp.maximum(1.0 - a1_up,  1e-10)
...
# ---------------------------------------------------------------------------
# THINC with local bounds (proper version — no w-blending limit)
# ---------------------------------------------------------------------------

def _thinc_alpha_he2024(a1_ext2, u_face, beta=4.0):
    """Proper THINC face reconstruction for alpha1.

    Unlike _thinc_face_anp (which uses w=4*Y_D*(1-Y_D) blending that limits
    compression), this version uses local bounds from upwind-upwind and
    acceptor cells to define the compression range.
    ...
    # Local bounds from upwind-upwind and acceptor
    q_lo = anp.minimum(q_UU, q_A)
    q_hi = anp.maximum(q_UU, q_A)
    dq = q_hi - q_lo

    # Normalized donor variable: a_norm in [0, 1]
    # Use large eps to avoid gradient overflow in autograd
    a_norm = (q_D - q_lo) / (dq + 1e-8)
    a_norm = anp.maximum(anp.minimum(a_norm, 1.0), 0.0)

    # THINC sigmoid compression
    q_thinc = q_lo + dq * (0.5 + 0.5 * anp.tanh(beta * (2.0 * a_norm - 1.0)))

    # Smooth activation based on local jump magnitude
    # Active (~1) when dq > ~0.02, inactive (~0) when dq < ~0.005
    w_act = anp.tanh(50.0 * dq)
...
def make_residual_he2024(Q_n, N, dx, dt, ph1, ph2, bc_l, bc_r,
                          ...
                          use_mmacm=False, mmacm_beta=2.9):
    ...
    def residual(Q):
        ...
        if use_thinc and live_thinc:
            ...
            # Standard 1st-order upwind / ghost extension (no THINC)
            vars_ext = [_ghost_anp(v, bc_l, bc_r)
                        for v in [a1r1, a2r2, ru, rE, p, c_wood]]
        ...
        #
        # Discrete:
        #   (α₁ - α₁ⁿ)/dt + (α₁_fR·u_fR - α₁_fL·u_fL)/dx
        #                  - (α₁+D₁)·(u_fR - u_fL)/dx = 0
        #
        # where:
        #   α₁_face = upwind (or THINC)
        #   u_face  = 0.5*(u_i + u_{i+1})

        u_face = u_face_thinc  # (N+1,) face velocities
        ...
        # div(u)/dx for cell i  (u_face already (N+1,))
...
def newton_he2024(Q_n, N, dx, dt, ph1, ph2, bc_l, bc_r,
    ...
    _thinc_cache = None
    if use_thinc and jacobian_method in ('fd_sparse', 'jfnk'):
        _thinc_cache = {
            'a1_face': _compute_thinc_face_from_Q(Q_n, N, bc_l, bc_r, thinc_beta)
        }

    # Residual functions:
    # - res_func: for autograd (w-blend THINC) or frozen-THINC (fd_sparse prec)
    # - res_func_live: for R evaluation and line search in fd_sparse/jfnk
    #   (live THINC, self-consistent — the actual system we want to solve)
    res_func = make_residual_he2024(
        ...
        use_mmacm=use_mmacm, mmacm_beta=mmacm_beta,
    )
    res_func_live = None
    ...
    for niter in range(max_newton):
        ...
        if jacobian_method == 'autograd':
            ...
            # Sparse row-column equilibration
            abs_J = abs(J_sp_fd)
...
def step_he2024(N, dx, Q_prev, ph1, ph2, bc_l, bc_r, cfg):
    ...
    mmacm_beta      = float(cfg.get('mmacm_beta', 2.9))

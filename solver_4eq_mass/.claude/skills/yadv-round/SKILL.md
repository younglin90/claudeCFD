...
- `cases.cpp` / `validation.cpp` — 라운드의 명시적 목표가 아닌 한 수정 금지 (게이트를 통과시키려고
  기준 바꾸는 것 금지).
- OFF 경로 (`ACID_YADV` unset) 은 항상 19/19 + `solver_denner` 발행 바이너리와 byte-identical 유지.
...
cd $W && DENNER_ACID=1 ./build-cpp/cpp/denner_1d/denner1d_validate                  # 19/19 필수
cd $W && python3 scripts/yadv_verify.py                                             # 9/9 byte-identical 필수
```
플랜이 정의한 추가 게이트(예: FD-invariance, 특정 케이스 회귀 없음)도 실행. 하드 게이트
(OFF 19/19+9/9) 실패 시 그 라운드는 **머지하지 않는다** — 원인이 명확하면 되돌리고 실패로 기록,

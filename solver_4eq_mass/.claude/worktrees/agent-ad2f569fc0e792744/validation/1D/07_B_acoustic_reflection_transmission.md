...
- `air_water_wiggle_ok`: Air-Water는 큰 임피던스 차이 때문에 전역 norm이 작아도 p/u에 보이는 저/중주파 wiggle이 남을 수 있으므로 별도 smooth-zone 기준을 추가한다.
  - Air-Water pressure `p_smooth_local_tv_excess <= 0.30`
  - Air-Water velocity `u_smooth_local_tv_excess <= 0.20`
  - Air-Water pressure `p_smooth_local_hf_max <= 0.04`
...
- `peak_amp_ok`: acoustic peak 진폭은 diffusion을 고려하되 과도한 overshoot를 막기 위해 검사한다.
  - Air-Water: exact peak 및 local packet peak amplitude ratio `0.85-1.10`
  - Helium-Air/Argon-Air: 약한 reflected/transmitted packet의 finite-N 오차를 고려해 `0.80-1.13`
  - 따라서 gas-gas 케이스는 전역 형상과 peak 위치가 좋으면 약한 packet의 10-20% 진폭 오차는 허용하되, Air-Water는 wiggle 기준을 별도로 엄격하게 적용한다.
...
#### denner_1d 07-B acoustic implementation note

07-B의 `denner_1d` 검증 경로는 **legacy Denner pressure-based 3N solver만** 사용한다. `linear_acoustic_fvm`, `linear_acoustic_inlet`, `five_eq_ad` 경로는 사용하지 않는다.

- **unknowns**: legacy 3N pressure-based variables `(p, u, T)` 또는 `(p, u, h)`
- **MWI**: Denner-type momentum-weighted face velocity / Rhie-Chow pressure-velocity coupling
- **acoustic face coupling**: Denner 논문 원형에서는 별도 impedance-balanced characteristic/Riemann face operator를 사용하지 않는다.
- **ACID**: stencil-local asymmetric face thermodynamics, with previous-time density/enthalpy also adjusted by the ACID stencil colour
- **VOF**: Denner compressible VOF `K` source enabled for acoustic interface motion
- **TVD limiter**: `Minmod`, same setting across Air-Water, Helium-Air, Argon-Air
- **적용 범위**: Air-Water, Helium-Air, Argon-Air 모두 동일
- **금지**: linear acoustic FVM shortcut, exact remap, subcase별 flux/limiter 전환, peak amplitude 후처리 보정

강화된 local packet peak 기준은 Denner 3N 경로에도 그대로 적용한다. 따라서 global norm이 작아도 reflected/transmitted wave packet amplitude가 exact와 크게 다르면 FAIL 처리한다.
...
| 15 | Air-Water `0.85 <= peak_amp_ratio <= 1.10`; Helium-Air/Argon-Air `0.80 <= peak_amp_ratio <= 1.13`; local packet peak도 동일 | diffusion-aware peak amplitude |
| 16 | `wave_symmetry <= 0.38` | wave shape symmetry |
| 17 | Air-Water wiggle guard: `p_smooth_local_tv_excess <= 0.30`, `u_smooth_local_tv_excess <= 0.20`, `p_smooth_local_hf_max <= 0.04` | Air-Water 비물리적 wiggle/수치진동 억제 |

...
PrimitiveState solve_case_acid(const CaseDefinition& c) {
    ...
    while (t < c.config.final_time && step < c.config.max_steps) {
        ...
        auto compute_R = [&]() {
            ...
            if (inlet_left) {
                const double ph = std::sin(2.0 * M_PI * c.inlet_frequency * (t + dt));
                uin = c.base_velocity + c.inlet_du * ph;
            }
            for (int f = 0; f <= n; ++f) {
                const int gL = f + 1, gR = f + 2;
                rho_f[f] = 2.0 / (1.0 / re[gL] + 1.0 / re[gR]);  // harmonic (ACID Eq.22, for MWI)
                const double ubar = 0.5 * (ue[gL] + ue[gR]);
                ...
                pface[f] = 0.5 * (pe[gL] + pe[gR]);
                const int gU = theta[f] >= 0.0 ? gL : gR;  // upwind cell (ghost idx)
                const double pU = std::max(pe[gU], 1.0), TU = std::max(Te[gU], 1e-6);
                const auto ppaU = phase_props(pU, TU, A);
                const auto ppbU = phase_props(pU, TU, B);
                raup[f] = ppaU.rho;
                rbup[f] = ppbU.rho;
                uconv[f] = ue[gU];
                const double Hkin = 0.5 * ue[gU] * ue[gU];  // upwind kinetic energy
            ...
            // --- exact residual R(u,p) (fills outer Rres) ---
            for (int i = 0; i < n; ++i) {
                const double trans_m = (s.rho[i] * s.u[i] - rho_o[i] * u_o[i]) * VdT;
                const double conv = mdotR[i] * uconv[i + 1] - mdotL[i] * uconv[i];
                const double pres = pface[i + 1] - pface[i];
                Rres[i][0] = trans_m + conv + pres;
                const double trans_c = (s.rho[i] - rho_o[i]) * VdT;
            ...
            if (coupled) {
                for (int i = 0; i < n; ++i) {
                    ...
                    const double trans_e = (s.rho[i] * s.h[i] - rho_o[i] * Htot_o[i]) * VdT;
        ...
        for (int it = 0; it < 40; ++it) {
            ...
            for (int i = 0; i < n; ++i) {
                MB[i][0][0] += std::max(s.rho[i], rho_floor) * VdT;
                ...
                MB[i][0][1] += s.drhodp[i] * s.u[i] * VdT;  // transient d(rho u)/dp
            ...
            if (std::getenv("ACID_ISOTHERMAL") == nullptr) {
                for (int i = 0; i < n; ++i) {
                    ...
                    const double fL = theta[i] * (ai * rHaup[i] + (1.0 - ai) * rHbup[i]);
                    const double adv = fR - fL;
                    const double rhoH = rho_o[i] * Htot_o[i] - dt / dx * adv + (s.p[i] - p_o[i]);

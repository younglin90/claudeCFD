...
def _acoustic_faces_muscl_np(u, p, Z, alpha, bc_l, bc_r, alpha_pure_tol, *,
    ...
    # FIVE_EQ_IMEX_ACOUSTIC_RECON selects the same-pure-face reconstruction:
    #   'component'      -> reconstruct p and u componentwise then Riemann-solve
    #                       at the reconstructed states (default; bit-identical
    #                       to the historical behaviour).
    #   'characteristic' -> reconstruct the LOCAL acoustic Riemann invariants
    #                       w+ = p + Z_f*u (right-going) / w- = p - Z_f*u
    #                       (left-going) and upwind each one per its own wave
    #                       direction.  Damps the marginally-resolved dispersive
    #                       band without the global dissipation a limiter switch
    #                       would impose.
    #   'bvd'            -> Deng/Xiao boundary-variation-diminishing selection:
    #                       per face pick the candidate (A = componentwise-MUSCL
    #                       high-order, B = first-order Z-Riemann-from-cells) that
    #                       minimizes the pressure total boundary variation against
    #                       the adjacent CELL values.  Binary argmin, zero tuned
    #                       constants; falls back to first order exactly where a
    #                       dispersive reconstruction overshoot is born.
    #   'muscl3'         -> limited kappa=1/3 third-order upwind-biased MUSCL on
    #                       the SAME same-pure faces the componentwise 2nd-order
    #                       path handles, then the SAME Z-Riemann solve.  Cuts the
    #                       4-5 point/wavelength dispersion at its source so the
    #                       spurious pressure packet born at the impedance jump
    #                       does not accumulate.  k=1/3 is the DERIVED unique value
    #                       giving third-order face accuracy in the van Leer
    #                       kappa-family (0.25 and the (1-+k) weights are scheme
    #                       algebra, not tuning).  Interface/mixed faces stay
    #                       componentwise.
    #   'weno3'/'weno5'  -> componentwise Jiang-Shu WENO reconstruction (see the
    #                       _weno3/5_face_left_np helpers) of p and u on the SAME
    #                       same-pure faces, then the SAME Z-Riemann solve.  WENO
    #                       attacks the AW dispersive-packet seed directly: far
    #                       lower phase error than TVD-MUSCL at 4-5 points per
    #                       wavelength AND smoothness-adaptive weights that
    #                       self-limit marginally-resolved content without a
    #                       global damping switch.  q_L comes from the left-biased
    #                       stencil of the left cell toward the face, q_R from the
    #                       mirror stencil of the right cell.  All weights/
    #                       indicators are literature-fixed JS constants (no
    #                       tuning).  WENO5 needs the wider 6-cell face window, so
    #                       it applies only where that window is in range AND one
    #                       resolved-pure material; otherwise the face keeps the
    #                       componentwise MUSCL reconstruction (NOT first order --
    #                       the change stays isolated).  Interface/mixed faces
    #                       stay componentwise; WAF stays component-only.
    acoustic_recon = os.environ.get(
        "FIVE_EQ_IMEX_ACOUSTIC_RECON", "component"
    ).strip().lower().replace("-", "_")
    if acoustic_recon not in {"component", "characteristic", "bvd", "muscl3",
                              "weno3", "weno5"}:
        raise ValueError(
            "FIVE_EQ_IMEX_ACOUSTIC_RECON must be 'component', "
            "'characteristic', 'bvd', 'muscl3', 'weno3', or 'weno5'.")
    # FIVE_EQ_IMEX_ACOUSTIC_DISS_CONSISTENT (H11c) -- parameter-free
    # dissipation-consistency correction.  The first-order Z-Riemann face
    # velocity u_f = (p_l - p_r + Z_l u_l + Z_r u_r)/(Z_l+Z_r) carries a
    # pressure-difference (Rhie-Chow) damping term (p_l - p_r)/(Z_l+Z_r).  On
    # same-pure HIGH-ORDER faces MUSCL shrinks that term to the reconstructed
    # jump (p_L - p_R)/(Z_l+Z_r) -- O(dx^2) on resolved smooth waves -- so the
    # 4-5 point/wavelength band loses its physical acoustic damping and a
    # spurious pressure packet (fed through beta*div_u in the p-row) grows.  We
    # add back exactly the damping reconstruction removed, i.e. the difference
    # between the cell-value pressure jump and the reconstructed pressure jump:
    #     u_f += ((p_l - p_r) - (p_L - p_R)) / (Z_l + Z_r).
    # Coefficient 1/(Z_l+Z_r) is the Riemann solver's own -- zero tuned
    # constants.  Resolved smooth waves -> O(dx^2) (no amplitude loss); 4-5 ppw
    # -> restores first-order damping; Nyquist (superbee returns zero slope) ->
    # p_L-p_R == p_l-p_r so the correction is zero (already fully damped).
    # Default off => bit-identical to historical behaviour.
    diss_consistent = _env_enabled(
        "FIVE_EQ_IMEX_ACOUSTIC_DISS_CONSISTENT", default="0")
    u = np.asarray(u, dtype=float)
    p = np.asarray(p, dtype=float)
    Z = np.asarray(Z, dtype=float)
    ...
    waf_enabled = (
        ...
        # WAF corrects the componentwise-reconstruction dispersion; in the
        # characteristic scheme the per-wave upwinding supersedes it, so skip.
        and acoustic_recon == 'component'
    )
    centered_interface = (
    ...
    if acoustic_recon == 'bvd':
        # BVD candidate B (low-order) = the first-order Z-Riemann face state that
        # _acoustic_faces_np built from cell values above.  Snapshot it before the
        # loop overwrites p_star/u_star with the high-order candidate A so both
        # candidates are available for the per-face argmin below.
        p_lo = p_star.copy()
        u_lo = u_star.copy()
    # --- H12b stencil-clean high-order gating (FIVE_EQ_IMEX_ACOUSTIC_STENCIL_CLEAN)
    # A face may keep the componentwise high-order reconstruction ONLY if its
    # entire 4-cell reconstruction stencil (ext cells f-1, f, f+1, f+2) is a
    # single resolved-pure material -- i.e. all three consecutive cell pairs are
    # same-pure by the existing `_same_pure_pair_np` logic (tol = max(
    # alpha_pure_tol, eps**0.25), no new constant).  Otherwise the reconstruction
    # slope crosses a material jump (the 3800:1 AW interface) and contaminates
    # div_u; the face then falls back to the exact first-order two-state
    # Z-Riemann `p_star/u_star` left by `_acoustic_faces_np`.  The fallback width
    # equals the reconstruction stencil width -- a stencil-derived binary rule,
    # zero tuned constants.  Default off => bit-identical.  Component/weno modes
    # (all use the same componentwise 4-cell footprint or wider); the other recon
    # branches untouched.
    stencil_clean_faces = None
    if (acoustic_recon in ('component', 'weno3', 'weno5')
            and _env_enabled("FIVE_EQ_IMEX_ACOUSTIC_STENCIL_CLEAN", default="0")):
        nf = len(high_face)
        L = len(alpha_ext)
        stencil_clean_faces = np.zeros(nf, dtype=bool)
        fmax = min(nf - 1, L - 3)  # need f-1 >= 0 and f+2 <= L-1
        if fmax >= 1:
            fs = np.arange(1, fmax + 1)
            clean = (
                _same_pure_pair_np(alpha_ext[fs - 1], alpha_ext[fs], alpha_pure_tol)
                & _same_pure_pair_np(alpha_ext[fs], alpha_ext[fs + 1], alpha_pure_tol)
                & _same_pure_pair_np(alpha_ext[fs + 1], alpha_ext[fs + 2], alpha_pure_tol)
            )
            stencil_clean_faces[fs] = clean
    # --- H15-WENO: WENO5 wider-window availability + material-clean gate --------
    # WENO5 reconstructs a face from the 6 ext cells f-2..f+3 (left state f-2..f+2,
    # right state f-1..f+3), so it may fire on a same-pure face ONLY where that
    # whole window is in range (f-2 >= 0 and f+3 <= L-1) AND every one of its five
    # consecutive cell pairs is same-pure (the reconstruction slope never crosses
    # the 3800:1 AW jump).  Elsewhere the face falls back to componentwise MUSCL
    # (handled by the elif chain).  Stencil-derived width, zero tuned constants.
    weno5_face_ok = None
    if acoustic_recon == 'weno5':
        nf = len(high_face)
        L = len(alpha_ext)
        weno5_face_ok = np.zeros(nf, dtype=bool)
        fmax = min(nf - 1, L - 4)  # need f-2 >= 0 and f+3 <= L-1
        if fmax >= 2:
            fs = np.arange(2, fmax + 1)
            clean = np.ones(len(fs), dtype=bool)
            for d in range(-2, 3):  # ext pairs (f+d, f+d+1) for f-2..f+3
                clean &= _same_pure_pair_np(
                    alpha_ext[fs + d], alpha_ext[fs + d + 1], alpha_pure_tol)
            weno5_face_ok[fs] = clean
    for f in np.flatnonzero(high_face):
        if f <= 0 or f + 2 >= len(p_ext):
            continue
        if stencil_clean_faces is not None and not stencil_clean_faces[f]:
            # H12b: this face's 4-cell reconstruction stencil crosses a material
            # jump -> keep the exact first-order two-state Z-Riemann face value.
            continue
        if acoustic_recon == 'characteristic' and same_face[f]:
            # Characteristic (Riemann-invariant) upwind reconstruction on a face
            # whose two neighbours are the SAME resolved pure material.  On such
            # a face the impedance Z varies only through the acoustic
            # perturbation itself (O(1e-4) relative), so the derived face
            # impedance Z_f = mean(Z_L, Z_R) is exact to that order and lets
            # each acoustic wave be upwinded per its own direction.
            Z_f = max(0.5 * float(Z_ext[f] + Z_ext[f + 1]), _EPS)
            # Invariants on the 4-point stencil (ext cells f-1, f, f+1, f+2):
            #   w+ = p + Z_f*u  (right-going),  w- = p - Z_f*u  (left-going).
            wp_fm1 = p_ext[f - 1] + Z_f * u_ext[f - 1]
            wp_f0 = p_ext[f] + Z_f * u_ext[f]
            wp_fp1 = p_ext[f + 1] + Z_f * u_ext[f + 1]
            wm_f0 = p_ext[f] - Z_f * u_ext[f]
            wm_fp1 = p_ext[f + 1] - Z_f * u_ext[f + 1]
            wm_fp2 = p_ext[f + 2] - Z_f * u_ext[f + 2]
            # w+ travels RIGHT -> its face value comes from the LEFT cell f.
            wp_face = wp_f0 + 0.5 * float(
                _tvd_pair(wp_f0 - wp_fm1, wp_fp1 - wp_f0, tvd_kind))
            # w- travels LEFT -> its face value comes from the RIGHT cell f+1.
            wm_face = wm_fp1 - 0.5 * float(
                _tvd_pair(wm_fp1 - wm_f0, wm_fp2 - wm_fp1, tvd_kind))
            # For constant Z this is algebraically the exact linear acoustic
            # Riemann solution evaluated with per-wave one-sided high-order
            # states; the 0.5 is the characteristic decomposition, Z_f is
            # derived (no tuned constant).  Reduces to the existing formula
            # when the invariant slopes vanish.
            p_star[f] = 0.5 * (wp_face + wm_face)
            u_star[f] = 0.5 * (wp_face - wm_face) / Z_f
            continue
        if acoustic_recon == 'muscl3' and same_face[f]:
            # Limited kappa=1/3 third-order upwind-biased MUSCL on a same-pure
            # face.  For the LEFT state extrapolated from cell f with
            # dm = q_f - q_{f-1}, dp = q_{f+1} - q_f:
            #   q_L = q_f + 0.25*((1-k)*L(dm,dp) + (1+k)*L(dp,dm)),
            # and for the RIGHT state from cell f+1 with dm2 = q_{f+1} - q_f,
            # dp2 = q_{f+2} - q_{f+1}:
            #   q_R = q_{f+1} - 0.25*((1-k)*L(dp2,dm2) + (1+k)*L(dm2,dp2)),
            # where L(a,b) = _tvd_pair(a,b,tvd_kind) is the existing limited
            # slope; the ARGUMENT-ORDER swap between the two terms builds the
            # upwind/downwind-biased pair (Hirsch/van Leer limited kappa form).
            # k=1/3 is the DERIVED unique value giving third-order face accuracy
            # in the van Leer kappa-family -- the 0.25 and (1-+k) weights are
            # scheme algebra, not tuned constants.  Note dm2 == dp (both are
            # q_{f+1} - q_f), so dp_* is reused for the right state.
            k = 1.0 / 3.0
            dm_p = p_ext[f] - p_ext[f - 1]
            dp_p = p_ext[f + 1] - p_ext[f]
            dp2_p = p_ext[f + 2] - p_ext[f + 1]
            dm_u = u_ext[f] - u_ext[f - 1]
            dp_u = u_ext[f + 1] - u_ext[f]
            dp2_u = u_ext[f + 2] - u_ext[f + 1]
            p_L = p_ext[f] + 0.25 * (
                (1.0 - k) * float(_tvd_pair(dm_p, dp_p, tvd_kind))
                + (1.0 + k) * float(_tvd_pair(dp_p, dm_p, tvd_kind)))
            p_R = p_ext[f + 1] - 0.25 * (
                (1.0 - k) * float(_tvd_pair(dp2_p, dp_p, tvd_kind))
                + (1.0 + k) * float(_tvd_pair(dp_p, dp2_p, tvd_kind)))
            u_L = u_ext[f] + 0.25 * (
                (1.0 - k) * float(_tvd_pair(dm_u, dp_u, tvd_kind))
                + (1.0 + k) * float(_tvd_pair(dp_u, dm_u, tvd_kind)))
            u_R = u_ext[f + 1] - 0.25 * (
                (1.0 - k) * float(_tvd_pair(dp2_u, dp_u, tvd_kind))
                + (1.0 + k) * float(_tvd_pair(dp_u, dp2_u, tvd_kind)))
        elif acoustic_recon == 'weno3' and same_face[f]:
            # WENO3-JS componentwise reconstruction on a same-pure face, feeding
            # the SAME Z-Riemann below.  4-cell window (ext f-1..f+2): the LEFT
            # state is the left cell f's left-biased stencil toward the face, the
            # RIGHT state the mirror stencil of the right cell f+1.  Same 4-cell
            # footprint as the componentwise MUSCL branch, so it composes with
            # STENCIL_CLEAN exactly as component mode does.
            p_L = _weno3_face_left_np(p_ext[f - 1], p_ext[f], p_ext[f + 1])
            p_R = _weno3_face_left_np(p_ext[f + 2], p_ext[f + 1], p_ext[f])
            u_L = _weno3_face_left_np(u_ext[f - 1], u_ext[f], u_ext[f + 1])
            u_R = _weno3_face_left_np(u_ext[f + 2], u_ext[f + 1], u_ext[f])
        elif (acoustic_recon == 'weno5' and same_face[f]
              and weno5_face_ok is not None and weno5_face_ok[f]):
            # WENO5-JS componentwise reconstruction on a same-pure face whose
            # wider 6-cell window (ext f-2..f+3) is in range AND one resolved-pure
            # material (weno5_face_ok, precomputed below).  LEFT state from cell
            # f's 5-cell left-biased stencil, RIGHT state from cell f+1's mirror
            # stencil, then the SAME Z-Riemann.  Non-clean / boundary faces fall
            # through to the componentwise MUSCL block below (NOT first order).
            p_L = _weno5_face_left_np(
                p_ext[f - 2], p_ext[f - 1], p_ext[f], p_ext[f + 1], p_ext[f + 2])
            p_R = _weno5_face_left_np(
                p_ext[f + 3], p_ext[f + 2], p_ext[f + 1], p_ext[f], p_ext[f - 1])
            u_L = _weno5_face_left_np(
                u_ext[f - 2], u_ext[f - 1], u_ext[f], u_ext[f + 1], u_ext[f + 2])
            u_R = _weno5_face_left_np(
                u_ext[f + 3], u_ext[f + 2], u_ext[f + 1], u_ext[f], u_ext[f - 1])
        elif same_face[f] or centered_interface:
        ...
        if diss_consistent and acoustic_recon == 'component' and same_face[f]:
            # H11c dissipation-consistency correction (derivation at the env
            # read above): restore the Rhie-Chow damping MUSCL removed on this
            # same-pure face by adding the cell-value minus reconstructed
            # pressure jump, scaled by the Riemann solver's own 1/(Z_L+Z_R).
            # Do NOT touch p_star -- the packet lives in the div_u -> p chain.
            u_star[f] += ((p_ext[f] - p_ext[f + 1]) - (p_L - p_R)) / den
        ...
        if acoustic_recon == 'bvd':
            # Boundary-variation-diminishing argmin (Deng/Xiao): compare the
            # PRESSURE JUMP at the face left by each candidate reconstruction
            # pair.  Candidate A = the componentwise-MUSCL pair (p_L, p_R) just
            # used for the Riemann solve; candidate B = the first-order cell
            # pair (p_ext[f], p_ext[f+1]).  In smooth monotone data the
            # reconstructed jump is O(dx^2) versus O(dx) for the cell pair, so
            # high-order wins; where reconstruction is manufacturing a spurious
            # extremum the reconstructed jump exceeds the cell jump and the
            # selector falls back to the entropic first-order Riemann face.
            # The SAME choice drives p and u so the face state stays a
            # consistent (p, u) pair.  Ties keep A.  Argmin only -- no
            # thresholds, no tuned constants.
            jump_A = abs(p_L - p_R)
            jump_B = abs(p_ext[f] - p_ext[f + 1])
            if jump_A > jump_B:
                p_star[f] = p_lo[f]
                u_star[f] = u_lo[f]
    if stencil_clean_faces is not None:
        # H12b: report the stencil-clean-masked high-order faces so the implicit
        # torch residual (ho_l/ho_r gating) and the theta-blended explicit
        # div_p_old/div_u_old both fall back to first order on the same faces.
        high_face = high_face & stencil_clean_faces
    return p_star, u_star, high_face
...
# --- H15-WENO: componentwise Jiang-Shu WENO reconstruction for the acoustic face
# WENO reconstructs a face value as a smoothness-adaptive nonlinear convex
# combination of candidate polynomial stencils.  Every constant here is the
# LITERATURE-FIXED Jiang-Shu value -- linear weights, smoothness-indicator
# coefficients, and the squared denominator -- so it carries the same
# "standard/derived, not tuned" standing as the existing superbee / THINC-beta /
# kappa=1/3 choices already in this file:
#     WENO3-JS  linear weights d0 = 1/3,  d1 = 2/3
#     WENO5-JS  linear weights d0 = 1/10, d1 = 6/10, d2 = 3/10
#     nonlinear weight  w_k = d_k / (eps + beta_k)^2   (JS power 2)
# The only deliberate (and commented) design choice is the eps floor.  The
# classic fixed eps=1e-6 is DIMENSIONAL and behaves differently on the O(1e5)
# pressure field than on the O(1e-2) velocity field.  A first attempt scaled it
# by the absolute stencil value squared (eps ~ 1e-6 * p^2 ~ 1e4); on the low-
# amplitude acoustic wave riding the 1e5 background that floor DOMINATES the
# smoothness indicators (beta ~ (dp)^2, O(1e1..1e3)) and degenerates WENO to the
# UNLIMITED optimal-linear reconstruction -> non-monotone -> the AW acoustic
# solve blows up.  The correct scale-invariant form makes eps relative to the
# LOCAL SOLUTION VARIATION rather than the absolute level: normalize the
# smoothness indicators by their own local maximum (a shift- AND scale-invariant
# quantity, degree-2 homogeneous like beta itself) and apply the standard
# dimensionless floor to the normalized indicators:
#     scale = max_k(beta_k, tiny) ;  w_k = d_k / (_WENO_EPS_REL + beta_k/scale)^2.
# _WENO_EPS_REL = 1e-6 is the standard JS floor, now dimensionless; _WENO_TINY is
# an absolute floor on `scale` so a perfectly constant stencil gives beta_k/scale
# = 0 -> w_k = d_k (linear, correct) with no 0/0 or overflow.
#
# NOTE: differentiating the adaptive weights (in the implicit acoustic Jacobian)
# is unstable on a near-equilibrium field -- see the straight-through gradient in
# `weno5_left_t` inside `_solve_acoustic_ad`, which keeps this VALUE but freezes
# the Jacobian to the linear-optimal 5th-order stencil.
_WENO_EPS_REL = 1.0e-6
_WENO_TINY = 1.0e-300


def _weno3_face_left_np(qm1, q0, qp1):
    """WENO3-JS reconstruction of the value at the RIGHT face of cell ``q0``.

    Two 2-point candidate stencils {qm1,q0} and {q0,qp1}:
        cand0 = -0.5*qm1 + 1.5*q0   (upwind-biased, extrapolates over the face)
        cand1 =  0.5*q0  + 0.5*qp1  (central)
    Smoothness beta0=(q0-qm1)^2, beta1=(qp1-q0)^2; linear weights d0=1/3, d1=2/3
    recover the third-order upwind face value where the data is smooth.  The
    RIGHT face state of the cell LEFT of a face uses the natural argument order;
    the LEFT face state of the cell RIGHT of a face is the MIRROR call
    ``_weno3_face_left_np(qp2, qp1, q0)``.
    """
    cand0 = -0.5 * qm1 + 1.5 * q0
    cand1 = 0.5 * q0 + 0.5 * qp1
    b0 = (q0 - qm1) ** 2
    b1 = (qp1 - q0) ** 2
    scale = max(b0, b1, _WENO_TINY)  # local variation^2; relative-eps normalizer
    a0 = (1.0 / 3.0) / (_WENO_EPS_REL + b0 / scale) ** 2
    a1 = (2.0 / 3.0) / (_WENO_EPS_REL + b1 / scale) ** 2
    s = a0 + a1
    return (a0 * cand0 + a1 * cand1) / s


def _weno5_face_left_np(qmm, qm, q0, qp, qpp):
    """WENO5-JS reconstruction of the value at the RIGHT face of cell ``q0``.

    Three 3-point candidate stencils centred on {q0}, linear weights
    d0=1/10, d1=6/10, d2=3/10, and the Jiang-Shu smoothness indicators.  The
    LEFT face state of the cell RIGHT of a face is the MIRROR call
    ``_weno5_face_left_np(qp3, qp2, qp1, q0, qm1)``.
    """
    c0 = (1.0 / 3.0) * qmm - (7.0 / 6.0) * qm + (11.0 / 6.0) * q0
    c1 = (-1.0 / 6.0) * qm + (5.0 / 6.0) * q0 + (1.0 / 3.0) * qp
    c2 = (1.0 / 3.0) * q0 + (5.0 / 6.0) * qp - (1.0 / 6.0) * qpp
    b0 = (13.0 / 12.0) * (qmm - 2.0 * qm + q0) ** 2 \
        + 0.25 * (qmm - 4.0 * qm + 3.0 * q0) ** 2
    b1 = (13.0 / 12.0) * (qm - 2.0 * q0 + qp) ** 2 \
        + 0.25 * (qm - qp) ** 2
    b2 = (13.0 / 12.0) * (q0 - 2.0 * qp + qpp) ** 2 \
        + 0.25 * (3.0 * q0 - 4.0 * qp + qpp) ** 2
    scale = max(b0, b1, b2, _WENO_TINY)  # local variation^2; relative-eps normalizer
    a0 = (1.0 / 10.0) / (_WENO_EPS_REL + b0 / scale) ** 2
    a1 = (6.0 / 10.0) / (_WENO_EPS_REL + b1 / scale) ** 2
    a2 = (3.0 / 10.0) / (_WENO_EPS_REL + b2 / scale) ** 2
    s = a0 + a1 + a2
...
def _solve_acoustic_ad(W_n, q1_new, q2_new, m_adv, alpha_new, dt,
    ...
    # --- FIVE_EQ_IMEX_ACOUSTIC_PURE_TOL_CONSISTENT (candidate H11a) -----------
    # Coefficient-consistency fix: the acoustic face masks
    # (_same_pure_material_face_mask / _pure_bulk_muscl_face_mask) treat a cell as
    # a resolved pure material using pure_tol = max(alpha_pure_tol, eps**0.25)
    # (~1.22e-4), but _phase_acoustic switches to the pure-phase EOS with the RAW
    # alpha_pure_tol (e.g. 1e-8).  An interface cell whose off-phase volume
    # fraction drifts into (alpha_pure_tol, eps**0.25) is therefore reconstructed
    # as pure but given a Wood-mixture beta/Z that dips (up to ~1.5% at the
    # air-water transmission cell), a single-cell impedance notch on the
    # non-dissipative CN branch.  Recomputing the acoustic coefficients with the
    # SAME derived pure_tol the masks use removes the notch.  eps**0.25 is a
    # derived constant already in the codebase (no new tuned parameter); inert
    # whenever alpha_pure_tol >= eps**0.25 (e.g. 02_A alpha_floor=1e-3).
    if os.environ.get("FIVE_EQ_IMEX_ACOUSTIC_PURE_TOL_CONSISTENT", "0").strip().lower() in {
            "1", "true", "on", "yes"}:
        _loose_tol = max(float(alpha_pure_tol), np.finfo(float).eps ** 0.25)
        if _loose_tol > float(alpha_pure_tol):
            rho_anchor, c_mix_sq, Z = _phase_acoustic(
                W_n, eos1, eos2, mixture_kind=mixture_kind,
                alpha_pure_tol=_loose_tol)
            beta = np.maximum(rho_anchor * c_mix_sq, _EPS)
    # --- end FIVE_EQ_IMEX_ACOUSTIC_PURE_TOL_CONSISTENT -----------------------
    ...
    # --- FIVE_EQ_IMEX_ACOUSTIC_ACID (candidate H12-ACID) ---------------------
    # Denner-Xiao-van Wachem 2018 (JCP 367:192) Sec.5 "Local Single-Phase
    # Assumption": at a material interface the acoustic face Riemann closure is
    # evaluated with the CELL's OWN impedance on both sides (Z_L = Z_R = Z_i)
    # instead of the two-side (Z_L, Z_R) pair.  This decouples the 3800:1
    # air-water impedance jump from the acoustic operator so the interface
    # transmits/reflects acoustics without the spurious near-Nyquist pressure
    # packet the asymmetric two-side Z-Riemann reconstruction generates during
    # the ~40-step interface crossing.  Ported from the frozen He2024 prototype
    # `explicit_mmacm_ex.py` `acid_interface` flag (~line 3874: "all face
    # impedances seen from cell i are evaluated with cell i's own (rho,c)").
    # Gated to the 2-cell band straddling each resolved pure|pure material face
    # using the SAME derived pure_tol = max(alpha_pure_tol, eps**0.25) topology
    # the `interface_be` scheme already uses -- no new tuned constant.  Default
    # off => bit-identical; composes with STENCIL_CLEAN (which additionally
    # drops the cross-jump high-order reconstruction on the same band).
    acid_enabled = _env_enabled("FIVE_EQ_IMEX_ACOUSTIC_ACID", default="0")
    acid_band = np.zeros(n, dtype=bool)
    if acid_enabled and n >= 2:
        _acid_tol = max(float(alpha_pure_tol), np.finfo(float).eps ** 0.25)
        _a_acid = np.asarray(alpha, dtype=float)
        _iface_acid = (
            ((_a_acid[:-1] <= _acid_tol) & (_a_acid[1:] >= 1.0 - _acid_tol))
            | ((_a_acid[1:] <= _acid_tol) & (_a_acid[:-1] >= 1.0 - _acid_tol))
        )
        acid_band[:-1] |= _iface_acid
        acid_band[1:] |= _iface_acid
    if acid_enabled and np.any(acid_band):
        # Explicit (Crank-Nicolson old-time) half: iter1 proved the packet is
        # carried in div_u_old (the previous step's converged faces re-enter
        # via (1-theta)*du_old).  Rebuild the divergence at band cells from
        # first-order cell values with the cell-local impedance Z_i on BOTH
        # faces (row-asymmetric: the shared interface face is seen as Z_water
        # from the water cell's row and Z_air from the air cell's row -- the
        # ACID property).  Interface faces are never same-pure, so the
        # componentwise high-order reconstruction that ran here reconstructed
        # across the material jump; the ACID cell-local Z-Riemann replaces it.
        if bc_l == 'periodic' and bc_r == 'periodic':
            _il_acid = (np.arange(n) - 1) % n
            _ir_acid = (np.arange(n) + 1) % n
        else:
            _il_acid = np.maximum(np.arange(n) - 1, 0)
            _ir_acid = np.minimum(np.arange(n) + 1, n - 1)
        _Zc_acid = np.maximum(np.asarray(Z, dtype=float), _EPS)
        _u_acid = np.asarray(z0_u, dtype=float)
        _p_acid = np.asarray(z0_p, dtype=float)
        # Left ACID face of cell i (Z_i on both sides): the (Z_l=Z_r=Z_c) limit
        # of the Z-Riemann closure -> arithmetic p, Z-weighted velocity jump.
        p_fl_acid = 0.5 * (_p_acid[_il_acid] + _p_acid) \
            + 0.5 * _Zc_acid * (_u_acid[_il_acid] - _u_acid)
        u_fl_acid = 0.5 * (_u_acid[_il_acid] + _u_acid) \
            + 0.5 * (_p_acid[_il_acid] - _p_acid) / _Zc_acid
        # Right ACID face of cell i.
        p_fr_acid = 0.5 * (_p_acid + _p_acid[_ir_acid]) \
            + 0.5 * _Zc_acid * (_u_acid - _u_acid[_ir_acid])
        u_fr_acid = 0.5 * (_u_acid + _u_acid[_ir_acid]) \
            + 0.5 * (_p_acid - _p_acid[_ir_acid]) / _Zc_acid
        div_u_old = np.where(acid_band, (u_fr_acid - u_fl_acid) / dx, div_u_old)
        div_p_old = np.where(acid_band, (p_fr_acid - p_fl_acid) / dx, div_p_old)
    # --- end FIVE_EQ_IMEX_ACOUSTIC_ACID --------------------------------------
    ...
    # --- parameter-free acoustic time-scheme selection ---------------------
    # FIVE_EQ_IMEX_ACOUSTIC_SCHEME selects the implicit time weighting:
    #   'theta'        -> default per-cell theta method (behaviour untouched)
    #   'interface_be' -> alpha-topology-gated BE band (L-stable at interface,
    #                     CN/BE elsewhere as the theta method already decides)
    #   'trbdf2'       -> one-step TR-BDF2 two-stage solve (L-stable, 2nd order)
    acoustic_scheme = os.environ.get(
        "FIVE_EQ_IMEX_ACOUSTIC_SCHEME", "theta").strip().lower().replace("-", "_")
    if acoustic_scheme not in {"theta", "interface_be", "trbdf2"}:
        raise ValueError(
            "FIVE_EQ_IMEX_ACOUSTIC_SCHEME must be 'theta', 'interface_be', "
            "or 'trbdf2'.")
    if acoustic_scheme == "interface_be":
        # Coefficient-free material-interface detection: a face is a material
        # interface when one side is a resolved pure phase and the other side
        # is the other resolved pure phase (the pure-jump criterion already used
        # elsewhere, cf. _pure_material_cell_mask / _same_pure_pair_np).  tol
        # follows the regime_auto convention max(alpha_pure_tol, eps**0.25) so it
        # is well defined even when alpha_pure_tol == 0 (no new tuned constant).
        pure_tol = max(float(alpha_pure_tol), np.finfo(float).eps ** 0.25)
        ...
        if a_arr.size >= 2:
            ...
            a_R = a_arr[1:]
            iface_face = (
                ((a_L <= pure_tol) & (a_R >= 1.0 - pure_tol))
                | ((a_R <= pure_tol) & (a_L >= 1.0 - pure_tol))
            )
            band_cell = np.zeros(n, dtype=bool)
            # The two cells straddling each interface face (cells i and i+1).
            band_cell[:-1] |= iface_face
            band_cell[1:] |= iface_face
            # Grow the band by one cell on each side.  The width-1 growth is
            # stencil-derived (the acoustic reconstruction is a 3-point stencil),
            # not a tuned band width.
            grown = band_cell.copy()
            grown[:-1] |= band_cell[1:]
            grown[1:] |= band_cell[:-1]
            band_cell = grown
            # Force BE (theta=1, L-stable) inside the band; leave the per-cell
            # theta method (wave->0.5 CN, flat->1.0 BE) untouched outside it.
            theta_cell = np.where(band_cell, 1.0, theta_cell)
    ...
    def build_sparse_jacobian_vectorized(theta_in=theta_cell, m_in=m_adv,
                                         p_anchor_in=p0, dp_old_in=div_p_old,
                                         du_old_in=div_u_old, dt_stage=dt):
        ...
        # H15-WENO: cells i-3 / i+3 -- only WENO5 reaches this wide; passed to the
        # residual through params (the 5-cell z-stencil is unchanged).
        illl = np.maximum(ill - 1, 0)
        irrr = np.minimum(irr + 1, n - 1)
        if bc_l == 'periodic' and bc_r == 'periodic':
            ill = (np.arange(n) - 2) % n
            irr = (np.arange(n) + 2) % n
            illl = (np.arange(n) - 3) % n
            irrr = (np.arange(n) + 3) % n
        z0_np = np.column_stack((
            z0_u[ill], z0_u[il], z0_u, z0_u[ir], z0_u[irr],
            z0_p[ill], z0_p[il], z0_p, z0_p[ir], z0_p[irr],
            # H15-WENO: i-3 / i+3 cells promoted from params INTO the Jacobian
            # stencil so jacrev captures the wide WENO5 coupling.  For every recon
            # mode that does NOT reach these cells (component / muscl3 / weno3 /
            # characteristic / bvd) the residual is independent of them, so their
            # jacrev columns are exactly zero and the assembled J is bit-identical.
            z0_u[illl], z0_u[irrr], z0_p[illl], z0_p[irrr],
        ))
        ...
        # --- H12b stencil-clean high-order gating (mirrors the explicit helper) ---
        # ho_l_i/ho_r_i (built from high_face) gate the implicit high-order face
        # overrides for BOTH p and u.  When STENCIL_CLEAN is active in component
        # mode, keep high-order on a face only if its full 4-cell reconstruction
        # stencil is one resolved-pure material: LEFT face of cell i uses cells
        # {ill, il, i, ir}, RIGHT face uses {il, i, ir, irr}.  This is idempotent
        # with the already stencil-clean-masked high_face returned by
        # _acoustic_faces_muscl_np, kept explicit so the torch masks stay correct
        # independently.  Zero constants; default off => bit-identical.
        ho_l_mask = high_face[:-1]
        ho_r_mask = high_face[1:]
        _recon_mode = os.environ.get(
            "FIVE_EQ_IMEX_ACOUSTIC_RECON", "component"
        ).strip().lower().replace("-", "_")
        if (_recon_mode in ('component', 'weno3', 'weno5')
                and _env_enabled("FIVE_EQ_IMEX_ACOUSTIC_STENCIL_CLEAN", default="0")):
            left_clean = same_ill_il & same_il_i & same_i_ir
            right_clean = same_il_i & same_i_ir & same_ir_irr
            ho_l_mask = ho_l_mask & left_clean
            ho_r_mask = ho_r_mask & right_clean
        # --- H15-WENO: WENO5 per-face availability + material-clean gate --------
        # WENO5 on cell i's LEFT face (i-1/2) needs the 6-cell window i-3..i+2 one
        # resolved-pure material; its RIGHT face (i+1/2) needs i-2..i+3.  Away from
        # a periodic domain the i-3/i+3 reach must also be in range, else the face
        # keeps componentwise MUSCL (weno5_*_ok = False).  Same derived-width,
        # zero-constant rule as the explicit helper's weno5_face_ok.
        same_illl_ill = _same_pure_pair_np(alpha_np[illl], alpha_np[ill], alpha_pure_tol)
        same_irr_irrr = _same_pure_pair_np(alpha_np[irr], alpha_np[irrr], alpha_pure_tol)
        if bc_l == 'periodic' and bc_r == 'periodic':
            avail_l = np.ones(n, dtype=bool)
            avail_r = np.ones(n, dtype=bool)
        else:
            _idx = np.arange(n)
            avail_l = (_idx - 3 >= 0) & (_idx + 2 <= n - 1)
            avail_r = (_idx - 2 >= 0) & (_idx + 3 <= n - 1)
        weno5_l_ok = (
            avail_l & same_illl_ill & same_ill_il & same_il_i
            & same_i_ir & same_ir_irr)
        weno5_r_ok = (
            avail_r & same_ill_il & same_il_i & same_i_ir
            & same_ir_irr & same_irr_irrr)
        params_np = np.column_stack((
            ...
            rho_star, beta, u0, p_anchor_in, m_in,
            u_mask.astype(float),
            dp_old_in, du_old_in, theta_in,
            left_boundary.astype(float),
            right_boundary.astype(float),
            ho_l_mask.astype(float),
            ho_r_mask.astype(float),
            ...
            acid_band.astype(float),
            # H15-WENO per-face availability masks (unused unless recon == weno5);
            # the i-3 / i+3 cell VALUES now live in the z stencil above (so their
            # jacrev columns are populated), not here.
            weno5_l_ok.astype(float), weno5_r_ok.astype(float),
        ))
        ...
        # FIVE_EQ_IMEX_ACOUSTIC_RECON: same 'component'/'characteristic'/'bvd'/
        # 'muscl3' switch used by the explicit face helper.  On same-pure faces the
        # implicit residual reconstructs the local acoustic Riemann invariants and
        # upwinds each per its wave direction ('characteristic'), per-face
        # argmin-selects the high- vs first-order face by pressure total boundary
        # variation ('bvd'), or reconstructs p/u with the limited kappa=1/3
        # third-order upwind-biased MUSCL then the same Z-Riemann ('muscl3', k=1/3
        # DERIVED for third-order face accuracy in the van Leer kappa-family), or
        # reconstructs p/u componentwise with Jiang-Shu WENO then the same
        # Z-Riemann ('weno3'/'weno5', literature-fixed JS weights/indicators);
        # default 'component' is bit-identical to the historical inline
        # reconstruction.
        acoustic_recon = os.environ.get(
            "FIVE_EQ_IMEX_ACOUSTIC_RECON", "component"
        ).strip().lower().replace("-", "_")
        if acoustic_recon not in {"component", "characteristic", "bvd", "muscl3",
            ...
            raise ValueError(
                "FIVE_EQ_IMEX_ACOUSTIC_RECON must be 'component', "
                "'characteristic', 'bvd', 'muscl3', 'weno3', or 'weno5'.")
        # FIVE_EQ_IMEX_ACOUSTIC_DISS_CONSISTENT (H11c): see the derivation in the
        # explicit _acoustic_faces_muscl_np env-read block.  Restores the
        # first-order Rhie-Chow damping ((p_cell_jump) - (p_recon_jump))/(Z_l+Z_r)
        # that componentwise MUSCL removes on same-pure high-order faces.  Only in
        # 'component' mode, only on same-pure faces (same_l_i/same_r_i masks).
        ...
        waf_acoustic = (
            os.environ.get("FIVE_EQ_IMEX_ACOUSTIC_WAF", "0")
            .strip().lower() in {"1", "true", "on", "yes"}
            # WAF corrects componentwise-reconstruction dispersion; the
            # characteristic scheme supersedes it, so disable WAF there.
            and acoustic_recon == 'component'
        )
        centered_interface = (
            os.environ.get("FIVE_EQ_IMEX_ACOUSTIC_INTERFACE_CENTERED", "1")
        ...
        def local_residual_torch(z, params):
            (u_ll, u_l, u_c, u_r, u_rr, p_ll, p_l, p_c, p_r, p_rr,
             u_lll_i, u_rrr_i, p_lll_i, p_rrr_i) = z
            (Z_l, Z_c, Z_r, c_l, c_c, c_r,
             ...
             same_il_i_i, same_ir_irr_i, acid_i,
             weno5_l_ok_i, weno5_r_ok_i) = params
            # FIVE_EQ_IMEX_ACOUSTIC_ACID (H12): freeze the LEFT and RIGHT face
            # impedances to this cell's own Z_c (Denner 2018 Sec.5 local
            # single-phase assumption).  Reassigned before den_l/den_r and every
            # face-Riemann formula below, so both the first-order and the
            # componentwise high-order face states use the ACID cell-local
            # impedance on band cells.  same-pure-only branches (characteristic /
            # muscl3 / waf / diss_consistent) never fire on interface band cells,
            # so their Z_f = mean(Z_l,Z_c) uses are unaffected there.
            Z_l = torch.where(acid_i > 0.5, Z_c, Z_l)
            Z_r = torch.where(acid_i > 0.5, Z_c, Z_r)
            ...
            def weno3_left_t(qm1, q0, qp1):
                # torch mirror of _weno3_face_left_np (RIGHT face of q0).  Same
                # literature-fixed JS constants and the relative (local-variation-
                # normalized) eps -- see the _weno3_face_left_np docstring block.
                cand0 = -0.5 * qm1 + 1.5 * q0
                cand1 = 0.5 * q0 + 0.5 * qp1
                b0 = (q0 - qm1) ** 2
                b1 = (qp1 - q0) ** 2
                scale = torch.clamp(torch.maximum(b0, b1), min=_WENO_TINY)
                a0 = (1.0 / 3.0) / (_WENO_EPS_REL + b0 / scale) ** 2
                a1 = (2.0 / 3.0) / (_WENO_EPS_REL + b1 / scale) ** 2
                s = a0 + a1
                return (a0 * cand0 + a1 * cand1) / s

            def weno5_left_t(qmm, qm, q0, qp, qpp):
                # torch mirror of _weno5_face_left_np (RIGHT face of q0).
                c0 = (1.0 / 3.0) * qmm - (7.0 / 6.0) * qm + (11.0 / 6.0) * q0
                c1 = (-1.0 / 6.0) * qm + (5.0 / 6.0) * q0 + (1.0 / 3.0) * qp
                c2 = (1.0 / 3.0) * q0 + (5.0 / 6.0) * qp - (1.0 / 6.0) * qpp
                b0 = (13.0 / 12.0) * (qmm - 2.0 * qm + q0) ** 2 \
                ...
                b2 = (13.0 / 12.0) * (q0 - 2.0 * qp + qpp) ** 2 \
                    + 0.25 * (3.0 * q0 - 4.0 * qp + qpp) ** 2
                m2 = torch.maximum
                scale = torch.clamp(m2(m2(b0, b1), b2), min=_WENO_TINY)
                a0 = (1.0 / 10.0) / (_WENO_EPS_REL + b0 / scale) ** 2
                a1 = (6.0 / 10.0) / (_WENO_EPS_REL + b1 / scale) ** 2
                a2 = (3.0 / 10.0) / (_WENO_EPS_REL + b2 / scale) ** 2
                s = a0 + a1 + a2
                weno = (a0 * c0 + a1 * c1 + a2 * c2) / s
                # Straight-through WENO Jacobian (implicit acoustic solve only).
                # jacrev of the ADAPTIVE Jiang-Shu weights is the PE-blow-up root
                # cause: on a pressure-equilibrium field the smoothness indicators
                # beta_k are roundoff noise, and the local-max normalizer makes
                # beta_k/scale an O(1) noise ratio whose derivative is a wrong,
                # realization-dependent (anti-diffusive) stencil.  Once multiplied
                # by the stiff-water pressure coupling (beta ~ rho c^2) in the p-u
                # Jacobian block, it amplifies interface roundoff into a blow-up.
                # The reconstructed VALUE stays the full nonlinear WENO5 (shock
                # capturing preserved in the residual), but the GRADIENT is frozen
                # to the linear-OPTIMAL 5th-order stencil d=(1/10,6/10,3/10) -- the
                # exact smooth-region limit of WENO and a standard stable high-order
                # operator -- by adding the (detached) nonlinear correction on top
                # of the differentiable linear-optimal reconstruction.  Parameter-
                # free; identical face VALUE to the explicit helper.
                lin = (1.0 / 10.0) * c0 + (6.0 / 10.0) * c1 + (3.0 / 10.0) * c2
                return lin + (weno - lin).detach()
            ...
            if acoustic_recon == 'muscl3':
                # Limited kappa=1/3 third-order upwind-biased MUSCL on the LEFT
                # face (cells l, c).  LEFT state from cell l with dm = p_l-p_ll,
                # dp = p_c-p_l:  q_L = q_l + 0.25*((1-k)*L(dm,dp) + (1+k)*L(dp,dm));
                # RIGHT state from cell c with dm2 = p_c-p_l, dp2 = p_r-p_c:
                # q_R = q_c - 0.25*((1-k)*L(dp2,dm2) + (1+k)*L(dm2,dp2)).  The
                # ARGUMENT-ORDER swap builds the upwind/downwind-biased pair.
                # k=1/3 is DERIVED (third-order face accuracy in the van Leer
                # kappa-family); 0.25 and (1-+k) are scheme algebra, not tuning.
                # Only same-pure faces switch; interface/mixed keep componentwise.
                k = 1.0 / 3.0
                p_lh_m3 = p_l + 0.25 * (
                    (1.0 - k) * limited_t(p_l - p_ll, p_c - p_l)
                    + (1.0 + k) * limited_t(p_c - p_l, p_l - p_ll))
                p_ch_l_m3 = p_c - 0.25 * (
                    (1.0 - k) * limited_t(p_r - p_c, p_c - p_l)
                    + (1.0 + k) * limited_t(p_c - p_l, p_r - p_c))
                u_lh_m3 = u_l + 0.25 * (
                    (1.0 - k) * limited_t(u_l - u_ll, u_c - u_l)
                    + (1.0 + k) * limited_t(u_c - u_l, u_l - u_ll))
                u_ch_l_m3 = u_c - 0.25 * (
                    (1.0 - k) * limited_t(u_r - u_c, u_c - u_l)
                    + (1.0 + k) * limited_t(u_c - u_l, u_r - u_c))
                p_lh = torch.where(same_l_i > 0.5, p_lh_m3, p_lh)
                p_ch_l = torch.where(same_l_i > 0.5, p_ch_l_m3, p_ch_l)
                u_lh = torch.where(same_l_i > 0.5, u_lh_m3, u_lh)
                u_ch_l = torch.where(same_l_i > 0.5, u_ch_l_m3, u_ch_l)
            if acoustic_recon == 'weno3':
                # WENO3-JS on the LEFT face (i-1/2, cells l,c).  q_L(minus) from
                # cell l uses {ll,l,c}; q_R(plus) from cell c uses the mirror
                # {r,c,l}.  Same same-pure gate as muscl3, feeding the same
                # Z-Riemann below.  The 4-cell footprint fits the torch 5-cell z
                # stencil exactly (no params needed) and is shared by the two
                # adjacent cells -> conservative & consistent.
                p_lh_w3 = weno3_left_t(p_ll, p_l, p_c)
                p_ch_l_w3 = weno3_left_t(p_r, p_c, p_l)
                u_lh_w3 = weno3_left_t(u_ll, u_l, u_c)
                u_ch_l_w3 = weno3_left_t(u_r, u_c, u_l)
                p_lh = torch.where(same_l_i > 0.5, p_lh_w3, p_lh)
                p_ch_l = torch.where(same_l_i > 0.5, p_ch_l_w3, p_ch_l)
                u_lh = torch.where(same_l_i > 0.5, u_lh_w3, u_lh)
                u_ch_l = torch.where(same_l_i > 0.5, u_ch_l_w3, u_ch_l)
            if acoustic_recon == 'weno5':
                # WENO5-JS on the LEFT face (cells l,c).  q_L(minus) from cell l
                # uses {lll,ll,l,c,r} (lll = i-3 passed via params); q_R(plus) from
                # cell c uses the mirror {rr,r,c,l,ll}.  Applied only where the
                # wider 6-cell window is in range AND one resolved-pure material
                # (weno5_l_ok_i); else the componentwise MUSCL state above is kept
                # (NOT first order -- change stays isolated).  jacrev differentiates
                # w.r.t. the 5-cell z only, so the i-3 coupling enters the residual
                # exactly (via params) but is dropped from the Jacobian -- an
                # intentional quasi-Newton step for this single limiter-frozen solve
                # of the (already nonlinear) WENO reconstruction.  Both adjacent
                # cells reconstruct the shared face from the same 6 cell values, so
                # the face state is consistent between neighbours.
                p_lh_w5 = weno5_left_t(p_lll_i, p_ll, p_l, p_c, p_r)
                p_ch_l_w5 = weno5_left_t(p_rr, p_r, p_c, p_l, p_ll)
                u_lh_w5 = weno5_left_t(u_lll_i, u_ll, u_l, u_c, u_r)
                u_ch_l_w5 = weno5_left_t(u_rr, u_r, u_c, u_l, u_ll)
                p_lh = torch.where(weno5_l_ok_i > 0.5, p_lh_w5, p_lh)
                p_ch_l = torch.where(weno5_l_ok_i > 0.5, p_ch_l_w5, p_ch_l)
                u_lh = torch.where(weno5_l_ok_i > 0.5, u_lh_w5, u_lh)
                u_ch_l = torch.where(weno5_l_ok_i > 0.5, u_ch_l_w5, u_ch_l)
            p_fl_ho = (Z_c * p_lh + Z_l * p_ch_l + Z_l * Z_c * (u_lh - u_ch_l)) / den_l
            u_fl_ho = (p_lh - p_ch_l + Z_l * u_lh + Z_c * u_ch_l) / den_l
            if diss_consistent and acoustic_recon == 'component':
                # H11c: restore the Rhie-Chow damping removed by componentwise
                # MUSCL on the LEFT face (cells l, c).  Cell-value jump p_l-p_c,
                # reconstructed jump p_lh-p_ch_l; add the difference / (Z_l+Z_c).
                # Only on same-pure faces (same_l_i); composes with the ho_l_i
                # gate downstream (u_fl_ho is selected only where ho_l_i>0.5).
                # p unchanged -- packet is in the div_u -> p chain.
                u_fl_corr = ((p_l - p_c) - (p_lh - p_ch_l)) / den_l
                u_fl_ho = u_fl_ho + torch.where(
                    same_l_i > 0.5, u_fl_corr, torch.zeros_like(u_fl_ho))
            if acoustic_recon == 'characteristic':
                # Left face (between cell i-1 and i): reconstruct the local
                # acoustic Riemann invariants w+- = p +- Z_f*u on the 4-point
                # stencil (ll, l, c, r) and upwind each per its wave direction.
                # Z_f = mean(Z_l, Z_c) is derived (on same-pure faces Z varies
                # only through the O(1e-4) acoustic perturbation itself).
                Z_f_l = torch.clamp(0.5 * (Z_l + Z_c), min=_EPS)
                wp_ll = p_ll + Z_f_l * u_ll
                wp_l = p_l + Z_f_l * u_l
                wp_cl = p_c + Z_f_l * u_c
                wm_l = p_l - Z_f_l * u_l
                wm_cl = p_c - Z_f_l * u_c
                wm_r = p_r - Z_f_l * u_r
                # w+ travels RIGHT -> face value from the LEFT cell (l);
                # w- travels LEFT  -> face value from the RIGHT cell (c).
                wp_face_l = wp_l + 0.5 * limited_t(wp_l - wp_ll, wp_cl - wp_l)
                wm_face_l = wm_cl - 0.5 * limited_t(wm_cl - wm_l, wm_r - wm_cl)
                p_fl_char = 0.5 * (wp_face_l + wm_face_l)
                u_fl_char = 0.5 * (wp_face_l - wm_face_l) / Z_f_l
                # Only same-pure faces switch to characteristic; interface /
                # mixed faces keep the componentwise+Riemann reconstruction.
                p_fl_ho = torch.where(same_l_i > 0.5, p_fl_char, p_fl_ho)
                u_fl_ho = torch.where(same_l_i > 0.5, u_fl_char, u_fl_ho)
            if waf_acoustic:
                Z_face_l = torch.clamp(0.5 * (Z_l + Z_c), min=_EPS)
                c_face_l = torch.clamp(0.5 * (c_l + c_c), min=_EPS)
                nu_l = torch.clamp(c_face_l * (dt_stage / dx), min=0.0, max=1.0)
            ...
            if acoustic_recon == 'bvd':
                # BVD (Deng/Xiao) per-face argmin on the LEFT face (cells l, c):
                # compare the pressure JUMP each candidate reconstruction pair
                # leaves at the face.  A = reconstructed pair (p_lh, p_ch_l),
                # jump O(dx^2) in smooth data; B = cell pair (p_l, p_c), jump
                # O(dx).  High-order wins in smooth monotone regions; where the
                # reconstruction manufactures a spurious extremum its jump
                # exceeds the cell jump and the face falls back to first order.
                # The SAME boolean drives p and u (consistent face state).  The
                # torch.where switch is a constant-direction subgradient so
                # jacrev stays well defined.  Ties keep A.
                tbv_A_l = torch.abs(p_ch_l - p_lh)
                tbv_B_l = torch.abs(p_c - p_l)
                pick_A_l = tbv_A_l <= tbv_B_l
                p_fl_hi = torch.where(pick_A_l, p_fl_ho, p_fl_raw)
                u_fl_hi = torch.where(pick_A_l, u_fl_ho, u_fl_raw)
            else:
                p_fl_hi = p_fl_ho
                u_fl_hi = u_fl_ho
            p_fl_raw = torch.where(ho_l_i > 0.5, p_fl_hi, p_fl_raw)
            u_fl_raw = torch.where(ho_l_i > 0.5, u_fl_hi, u_fl_raw)
            ...
            if acoustic_recon == 'muscl3':
                # Limited kappa=1/3 MUSCL on the RIGHT face (cells c, r).  LEFT
                # state from cell c with dm = p_c-p_l, dp = p_r-p_c:
                # q_L = q_c + 0.25*((1-k)*L(dm,dp) + (1+k)*L(dp,dm)); RIGHT state
                # from cell r with dm2 = p_r-p_c, dp2 = p_rr-p_r:
                # q_R = q_r - 0.25*((1-k)*L(dp2,dm2) + (1+k)*L(dm2,dp2)).  Same
                # DERIVED k=1/3 and same same-pure gating as the left face.
                k = 1.0 / 3.0
                p_ch_r_m3 = p_c + 0.25 * (
                    (1.0 - k) * limited_t(p_c - p_l, p_r - p_c)
                    + (1.0 + k) * limited_t(p_r - p_c, p_c - p_l))
                p_rh_m3 = p_r - 0.25 * (
                    (1.0 - k) * limited_t(p_rr - p_r, p_r - p_c)
                    + (1.0 + k) * limited_t(p_r - p_c, p_rr - p_r))
                u_ch_r_m3 = u_c + 0.25 * (
                    (1.0 - k) * limited_t(u_c - u_l, u_r - u_c)
                    + (1.0 + k) * limited_t(u_r - u_c, u_c - u_l))
                u_rh_m3 = u_r - 0.25 * (
                    (1.0 - k) * limited_t(u_rr - u_r, u_r - u_c)
                    + (1.0 + k) * limited_t(u_r - u_c, u_rr - u_r))
                p_ch_r = torch.where(same_r_i > 0.5, p_ch_r_m3, p_ch_r)
                p_rh = torch.where(same_r_i > 0.5, p_rh_m3, p_rh)
                u_ch_r = torch.where(same_r_i > 0.5, u_ch_r_m3, u_ch_r)
                u_rh = torch.where(same_r_i > 0.5, u_rh_m3, u_rh)
            if acoustic_recon == 'weno3':
                # WENO3-JS on the RIGHT face (i+1/2, cells c,r).  q_L(minus) from
                # cell c uses {l,c,r}; q_R(plus) from cell r uses the mirror
                # {rr,r,c}.  Same same-pure gate as the left face.
                p_ch_r_w3 = weno3_left_t(p_l, p_c, p_r)
                p_rh_w3 = weno3_left_t(p_rr, p_r, p_c)
                u_ch_r_w3 = weno3_left_t(u_l, u_c, u_r)
                u_rh_w3 = weno3_left_t(u_rr, u_r, u_c)
                p_ch_r = torch.where(same_r_i > 0.5, p_ch_r_w3, p_ch_r)
                p_rh = torch.where(same_r_i > 0.5, p_rh_w3, p_rh)
                u_ch_r = torch.where(same_r_i > 0.5, u_ch_r_w3, u_ch_r)
                u_rh = torch.where(same_r_i > 0.5, u_rh_w3, u_rh)
            if acoustic_recon == 'weno5':
                # WENO5-JS on the RIGHT face (cells c,r).  q_L(minus) from cell c
                # uses {ll,l,c,r,rr}; q_R(plus) from cell r uses the mirror
                # {rrr,rr,r,c,l} (rrr = i+3 via params).  Gated by weno5_r_ok_i
                # (wider window in range AND one resolved-pure material); else the
                # componentwise MUSCL state above is kept (NOT first order).  The
                # i+3 coupling enters the residual exactly but is dropped from the
                # Jacobian (see the left-face note).
                p_ch_r_w5 = weno5_left_t(p_ll, p_l, p_c, p_r, p_rr)
                p_rh_w5 = weno5_left_t(p_rrr_i, p_rr, p_r, p_c, p_l)
                u_ch_r_w5 = weno5_left_t(u_ll, u_l, u_c, u_r, u_rr)
                u_rh_w5 = weno5_left_t(u_rrr_i, u_rr, u_r, u_c, u_l)
                p_ch_r = torch.where(weno5_r_ok_i > 0.5, p_ch_r_w5, p_ch_r)
                p_rh = torch.where(weno5_r_ok_i > 0.5, p_rh_w5, p_rh)
                u_ch_r = torch.where(weno5_r_ok_i > 0.5, u_ch_r_w5, u_ch_r)
                u_rh = torch.where(weno5_r_ok_i > 0.5, u_rh_w5, u_rh)
            p_fr_ho = (Z_r * p_ch_r + Z_c * p_rh + Z_c * Z_r * (u_ch_r - u_rh)) / den_r
            u_fr_ho = (p_ch_r - p_rh + Z_c * u_ch_r + Z_r * u_rh) / den_r
            if diss_consistent and acoustic_recon == 'component':
                # H11c: same correction on the RIGHT face (cells c, r).
                # Cell-value jump p_c-p_r, reconstructed jump p_ch_r-p_rh; add
                # the difference / (Z_c+Z_r) on same-pure faces (same_r_i).
                u_fr_corr = ((p_c - p_r) - (p_ch_r - p_rh)) / den_r
                u_fr_ho = u_fr_ho + torch.where(
                    same_r_i > 0.5, u_fr_corr, torch.zeros_like(u_fr_ho))
            if acoustic_recon == 'characteristic':
                # Right face (between cell i and i+1): same invariant upwinding
                # on the 4-point stencil (l, c, r, rr) with Z_f = mean(Z_c, Z_r).
                Z_f_r = torch.clamp(0.5 * (Z_c + Z_r), min=_EPS)
                wp_lr = p_l + Z_f_r * u_l
                wp_cr = p_c + Z_f_r * u_c
                wp_rr = p_r + Z_f_r * u_r
                wm_cr = p_c - Z_f_r * u_c
                wm_rr = p_r - Z_f_r * u_r
                wm_rrr = p_rr - Z_f_r * u_rr
                # w+ face value from the LEFT cell (c); w- from the RIGHT cell (r).
                wp_face_r = wp_cr + 0.5 * limited_t(wp_cr - wp_lr, wp_rr - wp_cr)
                wm_face_r = wm_rr - 0.5 * limited_t(wm_rr - wm_cr, wm_rrr - wm_rr)
                p_fr_char = 0.5 * (wp_face_r + wm_face_r)
                u_fr_char = 0.5 * (wp_face_r - wm_face_r) / Z_f_r
                p_fr_ho = torch.where(same_r_i > 0.5, p_fr_char, p_fr_ho)
                u_fr_ho = torch.where(same_r_i > 0.5, u_fr_char, u_fr_ho)
            if waf_acoustic:
                Z_face_r = torch.clamp(0.5 * (Z_c + Z_r), min=_EPS)
                c_face_r = torch.clamp(0.5 * (c_c + c_r), min=_EPS)
                nu_r = torch.clamp(c_face_r * (dt_stage / dx), min=0.0, max=1.0)
            ...
            if acoustic_recon == 'bvd':
                # RIGHT face (cells c, r): same jump-based argmin as the left
                # face -- A = reconstructed pair (p_ch_r, p_rh), B = cell pair
                # (p_c, p_r).  Same choice drives p and u.
                tbv_A_r = torch.abs(p_rh - p_ch_r)
                tbv_B_r = torch.abs(p_r - p_c)
                pick_A_r = tbv_A_r <= tbv_B_r
                p_fr_hi = torch.where(pick_A_r, p_fr_ho, p_fr_raw)
                u_fr_hi = torch.where(pick_A_r, u_fr_ho, u_fr_raw)
            else:
                p_fr_hi = p_fr_ho
                u_fr_hi = u_fr_ho
            p_fr_raw = torch.where(ho_r_i > 0.5, p_fr_hi, p_fr_raw)
            u_fr_raw = torch.where(ho_r_i > 0.5, u_fr_hi, u_fr_raw)
            p_fl_b = torch.where(
            ...
            r_u = rho_i * u_c - m_i + dt_stage * div_p
            r_p = p_c - p0_i + dt_stage * (u0_i * dp_dx + beta_i * div_u)
        ...
        for i in range(n):
            col_ids = [
                ...
                # i-3 / i+3 columns (u then p) -- match the z stencil order
                # (u_lll, u_rrr, p_lll, p_rrr).  Zero for non-weno5 modes.
                illl[i], irrr[i], n + illl[i], n + irrr[i],
            ]
    ...
    def _linear_step(R_local, J_local):
        # One Newton step of the (limiter-frozen) linear acoustic operator from
        # the anchor guess y = (u0, p0).  Identical arithmetic to the original
        # inline solve, factored out so TR-BDF2 can reuse it per stage.
        scale = max(float(np.max(np.abs(J_local.diagonal()))), 1.0)
        from scipy.sparse import eye as speye
        dy = spsolve(
            J_local.tocsr() + 1.0e-12 * scale * speye(2 * n, format='csr'),
            -R_local)
        return y + dy

    if acoustic_scheme == "trbdf2":
        # One-step TR-BDF2 (L-stable, 2nd order).  The new schemes always use the
        # vectorized torch assembly, so the scalar N<32 branch below is reached
        # only by the default 'theta'/'interface_be' paths and stays intact.
        #
        # gamma = 2 - sqrt(2): the unique split that makes the two stage matrices
        # equal AND yields L-stability (the standard TR-BDF2 constant).
        gamma = 2.0 - np.sqrt(2.0)
        # Stage 1 -- trapezoidal rule over gamma*dt, anchored at W^n, using the
        # existing per-cell theta method (theta=0.5 in wave cells) -> (u*, p*).
        R1, J1 = build_sparse_jacobian_vectorized(dt_stage=gamma * dt)
        y1 = _linear_step(R1, J1)
        u_star = y1[:n]
        p_star = y1[n:]
        # Stage 2 -- BDF2 over the remaining (1-gamma)*dt.  Tableau coefficients:
        #   c  = (1-gamma)/(2-gamma)   implicit weight on L(.)
        #   a1 = 1/(gamma*(2-gamma))   weight on the stage-1 state
        #   a2 = -(1-gamma)^2/(gamma*(2-gamma))
        # a1 + a2 = 1, so uniform states are exact fixed points (PE preserved).
        # Structurally this is the theta=1 (BE) solve with dt_eff = c*dt and NO
        # old-divergence term, anchored at the blended state a1*state* + a2*state^n.
        # Coefficients (Z, beta, rho) stay frozen at W^n for BOTH stages.
        c_coef = (1.0 - gamma) / (2.0 - gamma)
        a1 = 1.0 / (gamma * (2.0 - gamma))
        a2 = -((1.0 - gamma) ** 2) / (gamma * (2.0 - gamma))
        m_star = rho_star * np.asarray(u_star, dtype=float)
        m_blend = a1 * m_star + a2 * np.asarray(m_adv, dtype=float)
        p_blend = a1 * np.asarray(p_star, dtype=float) + a2 * np.asarray(p0, dtype=float)
        theta_be = np.ones(n, dtype=float)
        zeros_div = np.zeros(n, dtype=float)
        R2, J2 = build_sparse_jacobian_vectorized(
            theta_in=theta_be, m_in=m_blend, p_anchor_in=p_blend,
            dp_old_in=zeros_div, du_old_in=zeros_div, dt_stage=c_coef * dt)
        y2 = _linear_step(R2, J2)
        # The downstream energy update recomputes the pressure-work faces from the
        # returned (u, p); returning the stage-2 solution makes those the stage-2
        # faces, as required for energy consistency.
        return y2[:n], y2[n:]

    if n >= 32:
        R, J = build_sparse_jacobian_vectorized()
        y_out = _linear_step(R, J)
        return y_out[:n], y_out[n:]
    ...
    for i in range(n):
        ...
        def local_res(z):
            ...
            if left_boundary:
                ...
                # FIVE_EQ_IMEX_ACOUSTIC_ACID (H12): freeze both face impedances
                # to cell i's own Z[i] on band cells (Denner 2018 Sec.5).
                _zl_acid = Z[i] if acid_band[i] else Z[il]
                p_fl, u_fl = face_local(p_l, p_c, u_l, u_c, _zl_acid, Z[i])
                p_l_eff = p_l
            if right_boundary:
                p_fr = p_c
                ...
                _zr_acid = Z[i] if acid_band[i] else Z[ir]
                p_fr, u_fr = face_local(p_c, p_r, u_c, u_r, Z[i], _zr_acid)
            p_r_eff = p_r
            dp_back = (p_c - p_l_eff) / dx
            dp_forw = (p_r_eff - p_c) / dx

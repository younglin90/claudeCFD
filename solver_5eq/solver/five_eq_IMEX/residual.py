...
def _acoustic_riemann_faces(W, p_L, p_R, u_L, u_R,
    ...
    if smooth > 0.0 and p_star.shape[0] > 2:
        ...
        p_sm = p_star.copy()
        u_sm = u_star.copy()
        # Damp grid-scale acoustic ringing without diffusing the whole
        # transmitted Gaussian.  The curvature sensor is near zero on smooth
        # profiles and near one on face-to-face oscillations.
        p_curv = np.abs(p_star[:-2] - 2.0 * p_star[1:-1] + p_star[2:])
        u_curv = np.abs(u_star[:-2] - 2.0 * u_star[1:-1] + u_star[2:])
        p_amp = np.abs(p_star[:-2]) + 2.0 * np.abs(p_star[1:-1]) + np.abs(p_star[2:]) + _EPS
        u_amp = np.abs(u_star[:-2]) + 2.0 * np.abs(u_star[1:-1]) + np.abs(u_star[2:]) + _EPS
        p_sensor = np.clip((p_curv / p_amp - 0.15) / 0.35, 0.0, 1.0)
        u_sensor = np.clip((u_curv / u_amp - 0.15) / 0.35, 0.0, 1.0)
        p_sm[1:-1] = p_star[1:-1] + w * p_sensor * (p_star[:-2] - 2.0 * p_star[1:-1] + p_star[2:])
        u_sm[1:-1] = u_star[1:-1] + w * u_sensor * (u_star[:-2] - 2.0 * u_star[1:-1] + u_star[2:])

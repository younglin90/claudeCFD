"""Benchmark Safe-NN-v4 (lean Anderson) on 5 cases."""
import time
import numpy as np
...
from solver_safe_nn_v4 import solve_safenn_v4
...
def main():
    ...
    for cid, N, kind in CASES:
        ...
        f, hist = solve_safenn_v4(case, max_outer=2000, tol=1e-7,
                                    anderson_m=5, safeguard_every=10,
                                    safeguard_ratio=1.3,
        ...
        if v == "WIN": wins += 1
        print(f"  v4 {cid:18s} total={total_lbe:>7d} wall={wall:>5.1f}s "

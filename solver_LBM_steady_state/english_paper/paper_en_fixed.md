...
$$\rho(\mathbf{x}) = \sum_{i} f_{i}(\mathbf{x}), \qquad \rho(\mathbf{x})\,\mathbf{u}(\mathbf{x}) = \sum_{i} \mathbf{c}_{i} f_{i}(\mathbf{x}), \qquad p(\mathbf{x}) = c_{s}^{2}\,\rho(\mathbf{x}). \qquad(1)$$
...
$$r_{p}^{k} = \big\| \widetilde{\delta p} \big\|_{\Omega_{f}}, \qquad r_{u}^{k} = \left\| \delta u_{x} \right\|_{\Omega_{f}}, \qquad r_{v}^{k} = \left\| \delta u_{y} \right\|_{\Omega_{f}}, \qquad \| \cdot \|_{\Omega_{f}}^{2} = \sum_{\mathbf{x} \in \Omega_{f}} ( \cdot )^{2}, \qquad(3)$$

$$r_{\mathrm{macro}}^{k} = \sqrt{ (r_{p}^{k})^{2} + (r_{u}^{k})^{2} + (r_{v}^{k})^{2} + (r_{w}^{k})^{2} }, \qquad I_{W}^{k} = \frac{r_{\mathrm{macro}}^{k-W} - r_{\mathrm{macro}}^{k}}{\max\!\left( r_{\mathrm{macro}}^{k-W},\, \varepsilon_{\mathrm{floor}} \right)} \le \eta. \qquad(4)$$
...
$$\text{converged} = \big[\, r_{\mathrm{macro}} \le C_{\mathrm{tol}}\,\tau \,\big] \ \text{AND}\ \big[\, \text{plateau}(r_{\mathrm{macro}}; W, \eta) = \text{true} \,\big] \ \text{AND}\ \big[\, \text{admissible}(f, \rho, u, v) = \text{true} \,\big]. \qquad(5)$$
...
$$\begin{bmatrix} J_{mm} & J_{mk} \\ J_{km} & J_{kk} \end{bmatrix} \begin{bmatrix} \delta m \\ \delta k \end{bmatrix} = - \begin{bmatrix} R_{m} \\ R_{k} \end{bmatrix}. \qquad(6)$$

Eliminating the kinetic block yields the Schur-complement problem for the conserved moments:

$$S_{m}\,\delta m = - \big( R_{m} - J_{mk}\, J_{kk}^{-1}\, R_{k} \big), \qquad S_{m} = J_{mm} - J_{mk}\, J_{kk}^{-1}\, J_{km}. \qquad(7)$$
...
$$Pf = \begin{pmatrix} \sum_{i} f_{i} & \sum_{i} c_{ix} f_{i} & \sum_{i} c_{iy} f_{i} \end{pmatrix} = \begin{pmatrix} \rho & \rho u_{x} & \rho u_{y} \end{pmatrix}, \qquad(8)$$

$$\left( P^{\dagger}\delta U \right)_{i} = w_{i}\left[\, \delta\rho + 3 c_{ix}\,\delta\left( \rho u_{x} \right) + 3 c_{iy}\,\delta\left( \rho u_{y} \right) \,\right]. \qquad(9)$$
...
$$S_{m} \approx P J_{f} P^{\dagger}, \qquad S_{m}\,\delta U \approx - P R\left( f^{*} \right), \qquad \delta f_{\mathrm{AP}} = P^{\dagger}\delta U. \qquad(10)$$
...
$$J_{f}(f)\,v \approx \frac{R(f + \epsilon v) - R(f)}{\epsilon}, \qquad \epsilon = \frac{10^{-7}\left( 1 + \left\| f \right\|_{2} \right)}{\left\| v \right\|_{2}}. \qquad(11)$$
...
$$S_{U}^{\mathrm{G}}(\mathbf{k}) = \mathsf{M} J(\mathbf{k}) \mathsf{T} = \mathsf{I}_{3} - \mathsf{M} A(\mathbf{k}) \mathsf{T}. \qquad(13)$$
...
$$S_{U}^{\mathrm{AP}}(\mathbf{k}) = S_{U}^{\mathrm{G}}(\mathbf{k}) - \kappa(\omega)\left[\, \mathsf{M} A(\mathbf{k})^{2} \mathsf{T} - \left( \mathsf{M} A(\mathbf{k}) \mathsf{T} \right)^{2} \,\right]. \qquad(14)$$
...
$$S_{U}^{\mathrm{reg}}(\mathbf{k}) = S_{U}^{\mathrm{AP}}(\mathbf{k}) + \eta\,\mathsf{I}_{3}, \qquad \eta = \frac{\sigma_{\max}\!\left( S_{U}^{\mathrm{AP}} \right)}{50}, \qquad(15)$$
...
$$B_{m} R_{f} = P^{\dagger}\,\mathcal{F}^{-1}\!\left\{ B_{U}(\mathbf{k}) \cdot \mathcal{F}\left[ P R_{f} \right](\mathbf{k}) \right\}. \qquad(16)$$
...
$$J_{f}\left( f^{k} \right)\delta f = - R\left( f^{k} \right) \quad \text{with preconditioner } M = B_{m},\ \ \text{operator } v \rightarrow J_{f}\left( f^{k} \right)v\ \text{(Eq. 11)},\ \ \text{restart} = 2 k_{\max},\ \ \text{maxiter} = 1. \qquad(17)$$
...
$$f_{\mathrm{trial}}(\alpha) = f^{k} + \alpha\,\delta f, \quad \alpha \in \left\{ 1, \tfrac{1}{2}, \tfrac{1}{4}, \tfrac{1}{8} \right\}, \quad \text{accept} \iff \text{admissible}(f_{\mathrm{trial}}) \ \wedge\ \left\| R\left( f_{\mathrm{trial}} \right) \right\| < \left\| R\left( f_{\mathrm{best}} \right) \right\| \ \wedge\ \text{conservation}(f_{\mathrm{trial}}). \qquad(18)$$
...
$$C_{\mathrm{round}} \approx \left( n_{G} + n_{\mathrm{trial}} \right) C_{G} + C_{\mathrm{FFT}}, \quad C_{\mathrm{FFT}} = \mathcal{O}\!\left( n_{m} N_{f}\log N_{f} \right), \quad M \approx q N_{f} + \mathcal{O}\!\left( n_{m} N_{f} \right) + \mathcal{O}\!\left( N_{b} \right), \qquad(19)$$
...
$$M^{n} = \sum_{\mathbf{x} \in \Omega_{f}} \rho^{n}(\mathbf{x})\,\Delta V, \qquad \epsilon_{M}^{n} = \frac{\left| M^{n} - M^{0} \right|}{\max\left( \left| M^{0} \right|, \epsilon \right)}, \qquad(21)$$

$$\epsilon_{Q}^{n} = \frac{\left| \sum_{\Gamma_{\mathrm{out}}} \int_{\Gamma} \mathbf{u}^{n} \cdot \mathbf{n}\, d\Gamma + \sum_{\Gamma_{\mathrm{in}}} \int_{\Gamma} \mathbf{u}^{n} \cdot \mathbf{n}\, d\Gamma \right|}{\max\!\left( \sum_{\Gamma_{\mathrm{in}}} \left| \int_{\Gamma} \mathbf{u}^{n} \cdot \mathbf{n}\, d\Gamma \right|, \epsilon \right)}. \qquad(22)$$
...
$$e_{\mathrm{ref}} = \frac{\| Q_h - Q_{\mathrm{ref}} \|_2}{\max\left( \| Q_{\mathrm{ref}} \|_2, \varepsilon_{\mathrm{ref}} \right)}. \qquad(23)$$

...
def make_paper():
    ...
    abstract = (
        ...
        "Re=100에서 RMS 중심선 속도 오차 3.7×10⁻³, Re=400에서 3.9×10⁻² 의 "
        "정확도를 보였다."
    )
    ...
    add_para(doc,
        ...
        "Re=100에서 SCMK 결과의 중심선 RMS 오차는 3.7×10⁻³ (수직), "
        "2.4×10⁻³ (수평)으로 기본 LBM의 3.0×10⁻³, 2.0×10⁻³와 매우 근접하다. "
        "Re=400에서 N=65 격자에서 SCMK는 RMS 오차 3.9×10⁻²로 기본 LBM의 "
        "2.0×10⁻²와 유사한 수준이다. 이 오차는 BGK의 압축성 효과와 격자 "
        "해상도 한계에 기인하며, 두 솔버 모두 동일한 정상해로 수렴함을 보인다.",
        indent_cm=0.5)
    ...
    rows = [
        ...
        ("Re=400", "65", "2.0×10⁻²", "3.9×10⁻²", "8.4×"),
        ("Re=400", "129", "5.0×10⁻²", "7.5×10⁻²", "14.1×"),
    ]
    for i, row in enumerate(rows, start=1):
        for j, val in enumerate(row):

...
from solver_novel5 import solve_pa_lgf
...
def main():
    ...
    for cid, N, kind in CASES:
        ...
        f, hist = solve_pa_lgf(case, max_outer=400, tol=TOL,
        ...
        print(f"  PA-LGF {cid:18s} accel={accel_lbe:>6d} tail={tail_lbe:>6d} "
              f"total={rows[-1]['total']:>7d} wall={wall:>5.1f}s rel_L2={sc['rel_L2']:.2e} conv={'Y' if conv else 'N'}")
    Path("paper_revision_data/v2_final/iter3_pa_lgf.json").write_text(
        json.dumps(rows, indent=2), encoding="utf-8")

    print("\n=== Iter3 PA-LGF vs Iter0 best ===")
    ...
    for r in rows:
        ...
        print(f"  {r['case']:18s} PA-LGF={r['total']:>7d}  best={best:>7d}  {v}")

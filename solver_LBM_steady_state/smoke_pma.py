"""Iter 6: pure macro Anderson smoke on 5 cases."""
import json, time
...
from solver_novel5 import solve_pure_macro_anderson
...
def main():
    for cid, N, kind in CASES:
        ...
        f, hist = solve_pure_macro_anderson(case, max_outer=400, tol=1e-7,
                                              anderson_m=10, verbose=False)
        ...
        print(f"  pmA {cid:18s} accel={accel:>6d} tail={tail:>6d} total={total:>7d} "
              f"rel_L2={rL2:.2e} conv={'Y' if conv else 'N'}  best_baseline={best:>6d}  {v}")

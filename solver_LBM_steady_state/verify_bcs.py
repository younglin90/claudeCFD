...
from solver_bcs_scmk import solve_bcs_scmk
...
def run_case(case_b, case_s, tol, baseline_runner, scmk_kw,
    ...
    t0 = time.perf_counter()
    f_s, hist_s = solve_bcs_scmk(case_s, tol=tol, verbose=False, **scmk_kw)
...
def main():
    tol = 1e-7
    scmk_kw = dict(max_outer=200, krylov_max=10, krylov_tol=1e-3, kinetic_substeps=15, rank=5)
    results = {}

...
def run_case(case_b, case_s, tol, baseline_runner, scmk_kw,
    ...
    S = build_spectral_schur(case_s.N, omega=case_s.omega, mode="ap")
    t0 = time.perf_counter()
    f_s, hist_s = solve_scmk(case_s, S, tol=tol, verbose=False, **scmk_kw)
...
def main():
    tol = 1e-7
    scmk_kw = dict(max_outer=80, krylov_max=10, krylov_tol=1e-3,

"""Verify robust SCMK on 5-case suite. No tuning."""
...
from solver_scmk_robust import solve_scmk_robust
...
def run_case(case_b, case_s, tol, baseline_runner, max_baseline=50000):
    ...
    t0 = time.perf_counter()
    f_s, hist_s = solve_scmk_robust(case_s, max_outer=80, tol=tol,
                                     krylov_max=10, krylov_tol=1e-3,
                                     line_search_max=5,
                                     kinetic_substeps_base=12,
                                     verbose=False)
...
def main():
    ...
    with open("verify_robust_log.json", "w") as fh:

"""Verify RRE-LBM (Koopman-DMD) on 5-case suite."""
...
from solver_rre import solve_rre_lbm
...
def run_case(case_b, case_s, tol, baseline_runner, max_baseline=50000):
    ...
    t0 = time.perf_counter()
    f_s, hist_s = solve_rre_lbm(case_s, max_cycle=30, tol=tol,
                                K=15, N_polish=5, verbose=False)
...
def main():
    ...
    with open("verify_rre_log.json", "w") as fh:

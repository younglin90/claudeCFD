...
from solver_sann import solve_sann
...
def run_case(case_b, case_s, tol, baseline_runner, scmk_kw,
    ...
    t0 = time.perf_counter()
    f_s, hist_s = solve_sann(case_s, tol=tol, verbose=False, **scmk_kw)
...
def main():
    tol = 1e-7
    scmk_kw = dict(max_outer=200, krylov_max=10, krylov_tol=1e-3, kinetic_substeps=15, anderson_m=5, newton_every=5, beta_max=0.7)
    results = {}

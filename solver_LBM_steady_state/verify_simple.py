...
from solver_simple import solve_simple
...
def run_case(case_b, case_s, tol, baseline_runner, scmk_kw,
    ...
    t0 = time.perf_counter()
    f_s, hist_s = solve_simple(case_s, tol=tol, verbose=False, **scmk_kw)
...
def main():
    tol = 1e-7
    scmk_kw = dict(max_outer=200, alpha_init=2.0,
                   kinetic_substeps=20, N_check=6, min_ratio=2.0)
    results = {}

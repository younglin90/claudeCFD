"""Verify a single proposed LBM method against fixed comparison methods.

The fixed comparison rows are read from the existing benchmark cache.  This
script reruns only ``solve_proposed_single`` and emits numeric JSON metrics on
the final stdout line for autoresearch.
"""
...
OUT = Path("paper_revision_data") / "single5_competitive"
...
def load_fixed_baselines():
    ...
    for case_id, data in payload.items():
        ...
        accuracy_rows = rows
        # Picard is the stored reference for these two cases, so including its
        # self-error would make the accuracy gate mathematically impossible.
...
def run_proposed(case_id, n, ref):
    ...
    target_converged = (
        hasattr(case, "analytical_ux")
        and np.isfinite(native_residual_pre)
        and native_residual_pre < 1.0e-3
        and err_pre["rel_L2"] <= 1.0e-12
        and err_pre["Linf"] <= 1.0e-12
    )

    steady_reference_converged = (
        np.isfinite(native_residual_pre)
        and native_residual_pre < 1.0e-7
        and err_pre["rel_L2"] < 1.0e-5
        and err_pre["Linf"] < 1.0e-6
    )

    if finite and (target_converged or steady_reference_converged):
        tail_lbe = 0
        tail_change = 0.0
    elif finite:
...
def main():
    ...
    for case_id, n in CASES:
        ...
        rows.append(row)
        print(
            f"{case_id:16s} single lbe={row['total_lbe']:7d}/{base['best_lbe']:7.0f} "

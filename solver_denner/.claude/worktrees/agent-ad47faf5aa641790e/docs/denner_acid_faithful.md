...
```
p_I/p_II = 1 + (2γ/(γ+1))(M_s²-1)·(1+Π̂/p_II)                       (59)  [MULTIPLIER, not divisor]
Π̂ = ((γ-1)/γ)(ρ_II cp,II T_II) - p_II        [=Π for single SG; =0 ideal]  (60)
ρ_I/ρ_II = [((γ+1)/(γ-1))(p_I+Π̂)/(p_II+Π̂)+1] / [((γ+1)/(γ-1))+(p_I+Π̂)/(p_II+Π̂)]   (61)
u_I = u_s(1 - ρ_II/ρ_I)                                             (62)
```
**CORRECTION (iter, case24 work)**: Eq.59 is a MULTIPLIER `(1+Π̂/p_II)`, NOT a divisor
`/(1+Π̂/p_II)`. Verified from the paper PDF (p.205 render) and against the paper's own
published single-phase water M_s=100 result p_I/p_II=7.0754e7 (the divisor form gave 4.65;
the multiplier form gives 7.077e7 ✓). The earlier divisor transcription was wrong.
**a_II = Eq.57 thermo-consistent mixture sound speed** = sqrt((γ_mix-1)·cp_mix·T) (Eq.57),
NOT the Wood/mechanical-equilibrium sound speed. For case24 (ψ=0.5 air-water, M_s=10):
a_II=642.7 m/s (Eq.57) vs 23.7 m/s (Wood) -- the Wood-based reference was 27x too slow.
Faithful case24 RH: Vs=6426.8, p_I=1.508e10, ρ_I=1857.3, u_I=4698.0. The solver's own
conservative single-T mixture-EOS Hugoniot at this Vs reproduces (p_I,ρ_I) to 1e-4 rel,
i.e. the solver and Eq.59-62 are self-consistent (the solver was failing only because the
reference was unfaithful).

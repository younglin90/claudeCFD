...
For current iterative probes, overwrite stable output files instead of creating
a new result directory for every run.  Use fixed scratch directories such as:

- `results/T-MLP-u/current_mach3_quick/`
- `results/T-MLP-u/current_three_case_quick/`
- `results/T-MLP-u/current_candidate/`

Only create new archival folders when the user explicitly asks for a preserved
run or when storing fixed baseline caches such as MLP-u1 comparison results.

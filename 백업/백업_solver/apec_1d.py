...
def rhs(U, scheme, dx, T_prev):
    ...
    # Base fluxes (APEC applies Eq. A.4 internally)
    ...
    if scheme == 'PEqC':
        # PEqC implementation (Eq. 54-56): Quasi-conservative energy equation
        ...
        # Override standard conservative energy update with Quasi-conservative RHS
...
def run(scheme, N=101, t_end=0.07, CFL=0.4, p_inf=5e6, k=15.0):
    ...
    print(f"\n{'='*55}\nScheme: {scheme}  N={N}  t_end={t_end:.3f}  CFL={CFL}  k={k}")
    ...
    return x, U, T, p, np.array(t_hist), np.array(pe_hist), np.array(en_hist)
...
def run_cpg(scheme, N=501, t_end=8.0, CFL=0.6, p0=0.9, k=20.0, flux='KEEP'):
    ...
    return x, U, p, np.array(t_hist), np.array(pe_hist), np.array(en_hist), diverged

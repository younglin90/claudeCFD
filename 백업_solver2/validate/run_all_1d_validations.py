...
Usage:
    python solver/validate/run_all_1d_validations.py
    python solver/validate/run_all_1d_validations.py --fast   # skip slow cases
"""
...
def v02_srk_ch4n2(fast=False):
    print("\n[V02] CH4/N2 SRK EOS interface advection (KEEP split-form, paper §3.2)")
    try:
        import apec
        # fast: N=51  t_end=0.05 (FC diverges ~0.021s, APEC survives)
        # full: N=101 t_end=0.07 (FC diverges ~0.037s, APEC survives ~0.082s)
        N      = 51   if fast else 101
        t_end  = 0.05 if fast else 0.07
        results = {}
        for sc in ('FC', 'APEC'):
            _, _, _, p, th, ph, _, div = apec.run(
                sc, N=N, t_end=t_end, CFL=0.6, k=15.0, flux='KEEP')
            results[sc] = (th, ph, div)
        th_ap, ph_ap, div_ap = results['APEC']
        th_fc, ph_fc, div_fc = results['FC']
        pe_apec = float(ph_ap[-1]) if len(ph_ap) else float('nan')
        pe_fc   = float(ph_fc[-1]) if len(ph_fc) else float('nan')
        # PASS: APEC survives (not diverged) AND either FC diverges or APEC PE < FC PE
        ok = (not div_ap) and np.isfinite(pe_apec) and (div_fc or pe_apec < pe_fc)
        # plot PE vs time
        try:
            fig, ax = plt.subplots(figsize=(8, 5))
            for sc, (th, ph, div) in results.items():
                lbl = f"{sc} ({'diverged' if div else f'PE={ph[-1]:.1e}'})"
                ax.semilogy(np.array(th)*1e3, ph, label=lbl)
            ax.set_xlabel('t  [ms]'); ax.set_ylabel('max |Δp/p₀|')
            ax.set_title(f'V02 SRK CH4/N2 KEEP  N={N}  k=15')
            ax.legend(); ax.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig(os.path.join(OUTPUT_DIR, f'V02_srk_ch4n2_N{N}.png'), dpi=120)
            plt.close()
        except Exception:
            pass
        return _record('V02_srk_ch4n2', ok,
                       f"APEC PE={pe_apec:.2e} div={div_ap}  FC PE={pe_fc:.2e} div={div_fc}")
    except Exception as ex:
...
def v06_sod_multicomponent(fast=False):
    ...
    try:
        import kinetic as kr
        from kinetic import IdealGasMixture, run_simulation, prim_to_cons_2s, cons_to_prim_2s
